# Running the Agent Memory Benchmark against Mnemosyne, without an external LLM API

**What this is.** A procedure for running AMB's own protocol — its datasets, its
document model, its prompts, its judging rule — against Mnemosyne, with **Claude
...
| The `mnemosyne` runtime image | `docker build -t mnemosyne .` from the repo root. |
...
deterministic — **no model touches it**. Mnemosyne is driven over `/v1`, exactly
as the real adapter drives it.

```bash
docker build -t mnemosyne .
docker volume create amb-data
docker run --rm -v amb-data:/data mnemosyne init
docker run -d --name amb-engine -v amb-data:/data --network mnemosyne_default \
  -e MNEMOSYNE_MCP_HTTP_TOKEN=bench \
  mnemosyne serve-http --host 0.0.0.0 --port 8080
```
