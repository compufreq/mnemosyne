...
4. **Drawer ids are deterministic** over (wing, room, source, chunk_index),
   ...
   **A save through an API is not.** `POST /v1/vaults/{id}/drawers`,
   `mnemosyne_save` and `mnemosyne_add_drawer` have no source to be a chunk
   ...
   only surface that takes it) or run `mnemosyne_dedup` / `mnemosyne dedup`;
...
- `--read-only` refuses all 13 mutating MCP tools and returns 403 on
  ...
  `MNEMOSYNE_READ_AUDIT=chain` — that variable's trail is empty on a
  ...
  `mnemosyne stats`, `mnemosyne_status` and `GET /v1/vaults/{id}/stats`.
...
- **External embeddings**: create the vault with
  ...
  so declaring `MNEMOSYNE_ADMISSION=quarantine` protected every vault
...
mnemosyne-orchestrator serve --read-replica --addr 0.0.0.0:8901
```
...
`MNEMOSYNE_FUSION_WEIGHT=0.70` still composes (digit-identical at the
...
All four are accepted on `POST /v1/vaults/{id}/search` and on `mnemosyne_search`
— the same key names, parsed by the same code. The CLI takes the one of them it
has a consumer for, `mnemosyne search --language <code>`, which selects the
...
`MNEMOSYNE_RERANK_TOP_N` (50) is a *latency cap* — one transformer forward
per candidate. `MNEMOSYNE_LATE_TOP_N` (**200**) is a *rescore depth* — MaxSim
...
`MNEMOSYNE_RERANK_TOP_N` still drives both stages, so a pinned deployment
...
you.** Mnemosyne screens writes and can quarantine what trips the
...
3. **A wing is the trust unit you can actually enforce.** Scope reads to
   ...
   `MNEMOSYNE_TRUST_FLOOR`) so a low-trust wing can neither answer nor
...
call.** A *search* result — `POST /v1/…/search` and `mnemosyne_search` —
...
/v1/vaults/{id}/drawers/{drawer_id}` or `mnemosyne_get_drawer` serialize
...
```bash
mnemosyne verify           # HMAC every record + replay the audit chain
...
the identical verdict. *Stated cost*: a wrong `MNEMOSYNE_PASSPHRASE` derives
...
mnemosyne bundle sign-keygen --out sign.key      # prints the pinnable sender once
mnemosyne export --to <recipient> --out palace.bundle --sign sign.key
mnemosyne import palace.bundle --identity ops.key --sender <sender-hex>
```
...
- **`/v1` has no KG *write* routes** except `POST …/kg/authority`. The KG is
  written by the CLI, by MCP (`mnemosyne_kg_add`) and by import; the REST
...
counted against an inventory (`crates/mnemosyne-cli/src/parity.rs`) in both
...
`mnemosyne_list_wings`/`_get_taxonomy` with its count: hiding a review queue's
...
| `mnemosyne_update_drawer` | W | replace content in place (re-sealed, audited; screened like a save when admission is on — a flagged update quarantines and the reply says so, the drawer keeps its previous content) |
| `mnemosyne_delete_drawer` | W | delete + tamper-evident tombstone. **Refused for a quarantine-pending drawer** on every surface, not only MCP: `admission allow`/`deny` are the doors, because a plain delete leaves only a `del/<id>` tombstone that nobody can tell from housekeeping |
| `mnemosyne_list_drawers` | | page drawer summaries; excludes the quarantine wing unless you name it (which MCP cannot) |
| `mnemosyne_delete_by_source` | W | delete everything mined from a source. Refuses the whole call — deleting nothing — if any of those drawers is awaiting an admission ruling |
| `mnemosyne_check_duplicate` | | is this exact content already filed? Quarantined rows do not answer: any writer can drive this oracle with content it chose, and answering would confirm that a screened write landed and hand back the quarantine id — the one thing the save path deliberately withholds from the writer |
...
`MNEMOSYNE_ASSERTION_SECRET` is set — on `/v1` **and** on `POST /mcp`, which
...
| POST | `/v1/vaults/{id}/refine` | distil verbatim drawers into receipted KG facts + searchable fact-drawers (needs `MNEMOSYNE_LLM_URL`). A fact is dated by the words in its note ("three months ago"), not by the note's own date: the extractor returns the span verbatim, the engine rejects any span the note does not contain and resolves the rest deterministically, falling back to `content_date`. The response reports `dated_from_text`. Every distilled fact records its **extractor identity** (the model that claimed it) inside the fact's HMAC — provenance an offline attacker cannot rewrite; facts added by hand carry none. **`mnemosyne refine` is the same code path** (`--wing`/`--room`/`--fact-room`/`--limit`/`--dry-run`), so the two surfaces build the same vault from the same `MNEMOSYNE_LLM_*` configuration; before v0.47.0 the CLI wrote no fact date, no grounding verdict and no searchable mirror |
...
`MNEMOSYNE_ORCH_ADMIN_TOKEN`; `GET /ui` serves the fleet
...
`MNEMOSYNE_LANG` (CLI language: en, de, es, fr, it, pt, ru, zh, ko, hi).

Models: `MNEMOSYNE_EMBEDDER` (`hash`|`onnx`|`ort`|`http`) ·
`MNEMOSYNE_EMBED_URL`/`_MODEL`/`_API`/`_KEY`/`_DIM`/`_CA` (served
embedder; TLS or loopback only, `_CA` pins a self-signed root) ·
`MNEMOSYNE_ONNX_MODEL`/`_TOKENIZER`/`_NAME` ·
`MNEMOSYNE_RERANKER` (`onnx`|`ort`|`colbert`|`colbert-ort`; the two
ColBERT values are **single-vault only** — `serve-http` refuses them,
same shape as `MNEMOSYNE_RETRIEVAL=hnsw`) ·
`MNEMOSYNE_RERANK_MODEL`/`_TOKENIZER`/`_NAME`/`_TOP_N` (50 — the
cross-encoder's latency cap: one transformer forward per candidate) ·
`MNEMOSYNE_LATE_TOP_N` (200 — the late-interaction *rescore depth*, a
separate knob because MaxSim is arithmetic over matrices built at ingest and
costs far less per candidate. Falls back to `MNEMOSYNE_RERANK_TOP_N` whenever
...
Retrieval: `MNEMOSYNE_RETRIEVAL` (`pq`|`fde`|`hnsw` — `hnsw` is an
...
`MNEMOSYNE_SEARCH_TRACE` (unset — any value prints a per-phase timing
...
too; unset it to turn it off) · `MNEMOSYNE_FUSION`
(`bm25` default |`legacy`; `rrf` removed — measured −7.3pp, warns and falls
back to `bm25`) · `MNEMOSYNE_FUSION_WEIGHT` (0.55 — the blend's semantic
...
value never per-query) · `MNEMOSYNE_TRUST_FLOOR` (unset — vault-level
...
`MNEMOSYNE_ADMIT_TRUSTED_SOURCES` (empty — comma list of surfaces whose
...
`MNEMOSYNE_ADMISSION` (off — `quarantine` screens every save with the
...
`MNEMOSYNE_ADMISSION_LLM` (unset — `advisory` wires the
`MNEMOSYNE_LLM_*` runtime as the screen's tier-2 classifier: consulted
...
`MNEMOSYNE_ADMISSION_RATE` (unset — `<count>/<seconds>` declares the
...
unscreened; consulted only when `MNEMOSYNE_ADMISSION=quarantine`) ·
`MNEMOSYNE_READ_AUDIT` (unset — `chain` appends one audit-chain record
...
`mnemosyne vault anchor <name>`) on a cadence of your own. **Not**
...
`MNEMOSYNE_TRAIN_SOURCE_CAP` (4 — per-wing cap divisor on global
...
`MNEMOSYNE_FTS_PREFILTER_MIN` (2048) ·
...
`MNEMOSYNE_POOL_DIV` (64 — semantic prefilters fetch at least `live/div`
...
`MNEMOSYNE_LLM_URL` (**TLS or loopback only** — cleartext http to a
...
must never cross a readable wire) · `MNEMOSYNE_LLM_MODEL`
(`llama3.2`) · `MNEMOSYNE_LLM_API` (`ollama`|`openai`) ·
`MNEMOSYNE_LLM_CA` (PEM whose certificates become the ONLY trust roots
for the LLM connection — the `MNEMOSYNE_EMBED_CA` pin one client over;
...
`MNEMOSYNE_INDEX_CA` (PEM whose certificates become the ONLY trust roots
...
vectors at rest for exactly that reason. `MNEMOSYNE_PGVECTOR_DSN` must
...
`MNEMOSYNE_ORCH_ADMIN_TOKEN` (required on the writer, ≥16 chars; unused
by `serve --read-replica`) · `MNEMOSYNE_ORCH_ADDR` (127.0.0.1:8900) ·
`MNEMOSYNE_ORCH_RATE_LIMIT` (req/min per tenant; unset/`0`/`off` = off;
...
`mnemosyne_save` refused — and `POST …/kg/authority` must 403 too, since
that is the route that had no guard when the guards were per-handler.
If you run with `MNEMOSYNE_ADMISSION=quarantine`, prove the fence as well:
