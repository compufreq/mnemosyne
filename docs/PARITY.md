...
| Model embeddings (sentence-transformers) | four postures — `mnemosyne-embed-onnx` (tract, pure Rust), `mnemosyne-embed-ort` (ONNX Runtime, ~2.5×/forward + int8), `http` (any served model, TLS-or-loopback enforced), or caller-supplied `external:<name>@<dim>`. Models are user-supplied throughout; see [EMBEDDERS.md](EMBEDDERS.md) |
...
| MCP stdio server (~35 tools) | 33 tools (daemon/sync/session tools inapplicable — process management moved to the OS). The count is not maintained by hand: `crates/mnemosyne-cli/src/parity.rs` holds the inventory and the code is counted against it **in both directions**, so a tool added without a line fails the build and a line naming a tool that no longer exists fails too |
...
- **Read and egress auditing** — exports are chain-audited unconditionally
  on every surface; reads are audited under `MNEMOSYNE_READ_AUDIT=chain`
