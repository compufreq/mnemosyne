...
| `POST /v1/vaults/{id}/refine` | LLM distillation of a drawer into KG facts (`MNEMOSYNE_LLM_*`; nothing is contacted unless a URL is set) |
...
`MNEMOSYNE_READ_AUDIT=chain`; and on the `/v1` side there were thirteen
...
- **The audit chain is not on the data plane, and that is a boundary rather
  ...
  `mnemosyne_history` on MCP, fenced by namespace and by the reserved review
...
- **The orchestrator CLI mirrors the admin plane** (`mnemosyne-orchestrator
...
`MNEMOSYNE_ADMIT_TRUSTED_SOURCES=cli`. `import` is deliberately its own
...
- `main.rs::reranker_factory` loads the model once: `MNEMOSYNE_RERANKER=onnx`
...
- **Read replicas** (`serve --read-replica`, v0.40.0): a replica opens
  ...
  enforces `MNEMOSYNE_ORCH_RATE_LIMIT` over its own traffic, so the
