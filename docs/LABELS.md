...
- **Cost tiers, now measured** (`mnemosyne-bench tagcost`, LoCoMo corpus,
...
- **`kind` on drawers** (consultation item 4) SHIPPED 2026-08-02, exactly
  ...
  (`mnemosyne_core::KIND_VOCAB`, validated at the single write choke
  ...
  (`mnemosyne-bench tagvalue`) shipped with it: R@1/R@5 + wrong-kind@1,
...
- **Trust labels** (ingest-time, deployment-assigned) SHIPPED 2026-08-03
  ...
  **candidate-set floor** (`min_trust` per request, `MNEMOSYNE_TRUST_FLOOR`
...
- The **quarantine wing** is this doctrine's hardest instance: a reserved
  ...
  operator-declared (`MNEMOSYNE_ADMISSION`), the write that lands there
