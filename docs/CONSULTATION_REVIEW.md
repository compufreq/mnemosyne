...
MemPalace and then proposed a target architecture for Mnemosyne: a typed
...
- **The density channel is closed at the draw (2026-08-03).** The named
  ...
  `1/MNEMOSYNE_TRAIN_SOURCE_CAP` (default 4) of every global codebook
...
- **Read-path auditing — CLOSED 2026-08-04.** The finding was correct: the
  ...
  **Reads are audited under a declaration** (`MNEMOSYNE_READ_AUDIT=chain`;
...
4. **Typed `kind` — shipped**, on the closed vocabulary
   `mnemosyne_core::KIND_VOCAB`, validated at the single write choke point
   ...
   **measured before it was claimed** (`mnemosyne-bench tagvalue`): on a
