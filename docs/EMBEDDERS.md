...
Every mnemosyne vault embeds text to power the semantic half of hybrid
...
MNEMOSYNE_EMBED_MODEL=bge-m3 docker compose run --rm embed-pull
...
MNEMOSYNE_EMBEDDER=http
MNEMOSYNE_EMBED_URL=https://embeddings-tls
MNEMOSYNE_EMBED_CA=/tls/caddy/pki/authorities/local/root.crt
MNEMOSYNE_EMBED_MODEL=bge-m3
```

Optional: `MNEMOSYNE_EMBED_API` picks the endpoint shape when probing
...
* **Cleartext http to a non-loopback host refuses at construction — no
  ...
  the required TLS infra; `MNEMOSYNE_EMBED_CA` pins its self-signed root
...
`ghcr.io/compufreq/mnemosyne:<tag>-ort` image (amd64 + arm64), each
smoke-probed at build for the compiled feature. Building yourself:

```sh
# build once with the feature compiled in
cargo build --release -p mnemosyne-cli --features onnx   # tract, pure Rust
cargo build --release -p mnemosyne-cli --features onnx,ort  # + ONNX Runtime

MNEMOSYNE_EMBEDDER=ort        # or onnx
MNEMOSYNE_ONNX_MODEL=/models/model.onnx
MNEMOSYNE_ONNX_TOKENIZER=/models/tokenizer.json
MNEMOSYNE_ONNX_NAME=bge-m3    # recorded as the vault's embedder identity
```

The posture that matches the sealed-vault promise in full: the model
runs inside the mnemosyne process, text never crosses a process
...
# produces model.onnx + tokenizer.json — point MNEMOSYNE_ONNX_MODEL/_TOKENIZER at them
```
...
you declare (`MNEMOSYNE_ONNX_NAME`) as the vault's embedder identity and
...
* **A model swap is manual, in both directions.** `hash` is
  `mnemosyne-hash-v3`; `onnx`/`ort` record `MNEMOSYNE_ONNX_NAME`; `http`
  records `http:<model>`, so the same refusal covers a served model too.
  Changing any of them means `MNEMOSYNE_FORCE_EMBEDDER=1` + `repair` —
...
`MNEMOSYNE_SEMANTIC_GATE` declares the gate (a cosine in `[0.0, 1.0]`;
`off` refuses semantic-only admission outright, i.e. lexical channels
only) and `MNEMOSYNE_SEMANTIC_FLOOR` the floor (a cosine in `[0.0, 0.98]`;
...
`MNEMOSYNE_FUSION_WEIGHT=0.70` still composes. Full per-pair tables and
