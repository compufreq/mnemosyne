...
1. **Candidate generation** — pull candidates through exactly one tier
   (`MNEMOSYNE_RETRIEVAL`, an `else if` chain with FDE first). Default:
...
`MNEMOSYNE_SEARCH_TRACE=1` prints per-phase milliseconds to stderr. It
...
**sealed** vault, hash embedder, `MNEMOSYNE_RETRIEVAL=pq`, k=5, ~200
...
`MNEMOSYNE_WING_PQ_MIN` (4096; `off` = no per-wing indexes) carry their
...
3. **Shipped: a two-stage pool.** Stage 1 fetches `live/64` ADC candidates
   (`MNEMOSYNE_POOL_DIV`; `off` restores the measured-leaky fixed floor).
...
| FTS5 BM25 prefilter | on-disk | hmac-only vaults ≥ `MNEMOSYNE_FTS_PREFILTER_MIN` (2k) | shipped |
| In-memory HNSW (`hnsw`) | O(corpus) | moderate corpora, raw speed | experimental |
| **On-disk PQ + IVF** (`set_pq`, `MNEMOSYNE_RETRIEVAL=pq`) | ~O(codebook) | large corpora, edge/IoT — **both vault levels**, sealed rows AEAD'd with a decrypt-once slab-grouped cache | **shipped** |
| + per-wing codebook/IVF (`MNEMOSYNE_WING_PQ_MIN`, 4096; `off`) | ~O(codebook) | wing-shaped build cost; scoped retrieval as a trust zone — **not** a query-latency win, settled at 10⁶ | shipped |
| + sealed page tier (`MNEMOSYNE_PQ_PAGE_MIN`) | ~O(working set) | sealed vaults past the RAM/open-time wall | shipped, **default off** |
| **MUVERA FDE** (`MNEMOSYNE_RETRIEVAL=fde`) | ~O(codes) | token-aware candidates ahead of ColBERT rescore | **shipped** |
| + FDE inverted tier (`MNEMOSYNE_FDE_IVF_MIN`) | ~O(codes) | past ~10⁶, only with containment validated on your corpus | shipped, **default off** (measured net-negative below that) |
