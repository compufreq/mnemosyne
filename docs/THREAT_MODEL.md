...
chain, and with `MNEMOSYNE_RETRIEVAL=pq` a search may still build or
...
chain-audited rulings, opt-in via `MNEMOSYNE_ADMISSION`) at the single
...
| Read/egress audit | `egress/export` chain record on every export, behind no declaration (a read-only replica warns and serves unaudited); `MNEMOSYNE_READ_AUDIT=chain` records each search with a **keyed** query fingerprint, never text | A7 forensics; insider/exfil accounting |
...
Under `MNEMOSYNE_READ_AUDIT=chain` each search appends a record too,
...
### Zone 1 — the memory store (mnemosyne owns this)
...
- **Admission check at ingest** (BUILT; opt-in via
  `MNEMOSYNE_ADMISSION=quarantine` — screening changes what a save
  ...
  **single write choke point** every write funnels through, and every
  ...
  (`MNEMOSYNE_ADMISSION_RATE` — the tier-1 signal candidate bytes
  ...
  (`MNEMOSYNE_ADMISSION_LLM=advisory`) can push toward quarantine and
...
- **Quarantine wing** (BUILT) — flagged writes divert sealed into the
  ...
  **MCP the wing is not reachable at all**: one fence over the raw
  ...
  operator act (`mnemosyne admission …`, `GET /v1/vaults/<id>/admission`),
  ...
  (`MNEMOSYNE_ADMIT_TRUSTED_SOURCES`). A diverted save **says so on
...
is read by the agent's LLM. mnemosyne can *offer* the defenses but
...
- **Data-not-instructions delivery** — retrieval returns memory as a
  ...
  either surface (`POST /v1/…/search`, `mnemosyne_search`) carries the
  ...
  /v1/vaults/{id}/drawers/{drawer_id}`, `mnemosyne_get_drawer`), which
...
- **Trust-class gating** — deployment-assigned wing trust
  ...
  (`MNEMOSYNE_TRUST_FLOOR`), resolved before candidates are drawn so a
...
The honest line: **mnemosyne cannot force an LLM to respect this
...
control**, not a mnemosyne feature.

### The one guarantee that holds across all three zones

mnemosyne is an **inert store**: it never executes retrieved content,
...
poisoned record cannot make mnemosyne do anything. The danger is
entirely downstream, in components we are honest about not being.

The posture, stated once: mnemosyne provides the materials to defend
