...
Mnemosyne's cryptography is symmetric wherever data rests. Grover's
...
C3.4, `mnemosyne bundle keygen` produces a **hybrid identity** —
...
```text
MNEMOSYNE-BUNDLE-2 ‖ eph_x25519_pub (32) ‖ mlkem_ct (1088) ‖ nonce (24) ‖ ciphertext
file_key = HKDF-SHA256(salt = eph_pub ‖ recipient_x_pub,
                       ikm  = DH(eph, recipient_x) ‖ kem_shared,
                       info = "mnemosyne.v2/bundle")
```
...
|---|---|---|
| v1 (`MNEMOSYNE-BUNDLE-1`) | opens | opens (curve half) — upgrading an identity never orphans old backups |
| v2 (`MNEMOSYNE-BUNDLE-2`) | typed refusal naming the hybrid format | opens |
...
hop alike; nothing in mnemosyne needs to change for it.
...
Nothing in mnemosyne processes anything on a quantum computer.
