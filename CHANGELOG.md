...
The exploit, and it worked: `MNEMOSYNE_ADMISSION=quarantine` diverts a poisoned
...
- **On MCP, fenced — not raw.** `mnemosyne_history` (34 tools now) answers an
  ...
  each one.
- **CLI `mnemosyne history` and `GET /v1/vaults/{id}/history`** answer at
...
- **The console could not open the drawer it had just listed** (C1), and
  **reported success for writes that were diverted or unattested** (C2). It
  ...
  `mnemosyne admission list`, which prints ids, wings, signal codes and no
  content — it names `mnemosyne drawer get <id>` now — and the by-id review
...
- **CLI import verified a signature only when `--sender` was passed** (C5).
  ...
  `mnemosyne-vault::bundle::attest`, which both call.
...
- **The fleet's operator plane was reachable only by curl** (C9): ten routes
  ...
  mirrors the plane. `mnemosyne-orchestrator ops <tenant> <op>` now does,
...
- **The plaintext half is a required argument**, not a flag with a default:
  ...
  surface says `Refuse`. `mnemosyne index push --allow-plaintext` is the
...
- **The transport half is the engine's existing rule, applied here**: TLS or
  ...
  `mnemosyne-net`, because it was implemented once in `mnemosyne-llm` for
...
- **`MNEMOSYNE_INDEX_CA` pins a self-signed root**, replacing the public
...
- **`PalaceStore::tighten_anchor`**, reachable as `mnemosyne vault anchor
...
- Also fixed in passing, since the surface was open: `mnemosyne vault status`
...
- **`mnemosyne_kg_add` reached the authority tier's outcomes, and the fence
...
- **`mnemosyne refine` was a second distillation implementation again — a
  ...
  reserved wing the moment a wing is named, so `mnemosyne refine --wing
  quarantine-pending` lifted pending review evidence into the graph where
  `mnemosyne_kg_query` serves it. The gate against a third round is
...
- **`mnemosyne_follow_tunnel` walked the MCP quarantine fence.** The fence
  ...
  **dereferences** to one: `follow_tunnel` resolves a destination and calls
  ...
  predate the guard.
- **`mnemosyne_kg_set_authority` was on the agent surface**, and `kg_import`
...
- **A stored, self-rearming denial of service.** `meta.filed_at` is
  ...
  imported drawer panics `mnemosyne_get_closet_index`, a session-start
...
`MNEMOSYNE_ORCH_RATE_LIMIT=3`, so anything after it answers 429. The
...
- **The prevention layer** (`crates/mnemosyne-cli/src/parity.rs`): the MCP
...
- Honest residual, recorded not dressed up: a read-only store with
  `MNEMOSYNE_RETRIEVAL=pq` still **writes**, because the PQ tier builds its
...
- Numbers corrected by counting: the battery is **551 run / 4 ignored / 555
  ...
  method and is recorded beside the corrected one); **76** `MNEMOSYNE_*`
...
- **The `/mcp` store on a `serve-http --read-only` server was opened
  ...
  `MNEMOSYNE_READ_AUDIT=chain` on and appended a chain record per `/mcp`
...
- **Key rotation from `/v1` corrupted the co-resident `/mcp` handle.**
  ...
  `mnemosyne vault rotate <name>`). Other tenant vaults are untouched —
...
- **Pending review evidence cannot be destroyed except by ruling on it.**
  ...
  DELETE path was open. `mnemosyne_delete_drawer` over MCP — the surface
...
- **`MNEMOSYNE_ASSERTION_SECRET` now covers `POST /mcp`.** Every `/v1`
...
- **The admin console grew an OPS tab.** `ui.html` had zero occurrences
  ...
  `MNEMOSYNE_ADMISSION=quarantine` gave an operator a console that never
...
- **`verify` now covers drawer supersession receipts on every surface.**
  ...
  `mnemosyne_verify` made. `POST /v1/vaults/{id}/verify` — and the admin
...
- **`diary_write` handed the caller's `agent` argument straight into
  ...
  and a caller cannot set it". With `MNEMOSYNE_ADMIT_TRUSTED_SOURCES=cli`
...
- **Both import surfaces let a payload set `added_by`.** They deserialize
  ...
  `MNEMOSYNE_ADMIT_TRUSTED_SOURCES=import`.
- **Bulk ingest reported nothing about what it quarantined.**
  ...
  so `mnemosyne import` printed "imported 500" while an arbitrary number
...
- **`MAX_CONTENT_BYTES` is the engine's bound, not one command's.** It
  was enforced only by `mnemosyne remember`; MCP and `/v1` accepted
...
rather than one handler at a time (`crates/mnemosyne-cli/src/search.rs`,
...
- **The CLI can declare its language** (`--language`, the same 13-value
  ...
  `mnemosyne search`, which returned strictly fewer hits than the same
...
- **`refine` is one implementation now** (`crates/mnemosyne-cli/src/refine.rs`),
  driven identically by `mnemosyne refine` and `POST /v1/vaults/{id}/refine`.
  The same `MNEMOSYNE_LLM_*` configuration built two different vaults
...
- **Stats report the committed chain, never a handle's cached anchor.**
  ...
  `mnemosyne_status` and the telemetry sampler now answer the same number at
  the same instant.
...
- **`mnemosyne_chain_commits_total` counts records, not anchors.** Its
  ...
  `POST /v1/…/import` and 4 times through `mnemosyne import`, and read-audit
...
- **`MNEMOSYNE_ORCH_RATE_LIMIT` refuses garbage instead of disabling
...
- **Documentation corrected against the code** (each of these was wrong in
  the reference an operator would consult): `MNEMOSYNE_ORT_POOL` defaults to
  **cores**, not 1, and the pool holds one model copy per slot — a 8–64×
  memory under-estimate; `MNEMOSYNE_TOK_PQ_MIN` / `MNEMOSYNE_FDE_PQ_MIN`
  default to **256**, not "off", so two quantization tiers self-activate
  while the architecture page framed them as opt-in; `MNEMOSYNE_EMBEDDER`
  accepts **`http`**, the one served-embedder posture that needs no
  feature-gated build; `MNEMOSYNE_SEARCH_TRACE` is documented for the first
  time, including that it is **presence-triggered** (`0` and `off` turn it
  ON); `MNEMOSYNE_RETRIEVAL=hnsw` and `MNEMOSYNE_RERANKER=colbert*` are
...
- **Reads are chain-audited under a declaration**
  (`MNEMOSYNE_READ_AUDIT=chain`, 74th env var, unset = the
...
- **`bundle keygen` is hybrid X25519 + ML-KEM-768 by default** (FIPS
  ...
  (`MNEMOSYNE-BUNDLE-2` ‖ eph_pub ‖ kem_ct(1088) ‖ nonce ‖ ct) derives
  its file key from BOTH shared secrets — `HKDF-SHA256(ikm = DH ‖
  kem_shared, info = "mnemosyne.v2/bundle")` — so an attacker must
...
- **The binary smoke now runs against the PACKAGED layout, not the
  ...
  unchanged and unweakened: `--help`, then `MNEMOSYNE_EMBEDDER=ort`
...
- **Attack-fixture similarity** joins the deterministic tier-1 admission
  ...
  attack payload shapes (`mnemosyne_core::admission::ATTACK_FIXTURES` —
...
- **The declared per-writer rate screen** (`rate-anomaly` signal,
  `MNEMOSYNE_ADMISSION_RATE=<count>/<seconds>`, 73rd env var, unset =
...
- Store: `set_admission_rate` beside `set_admission`; the rate signal
  ...
  above is inert unless `MNEMOSYNE_ADMISSION=quarantine` is on, and
...
- **Releases ship the `ort` posture ready-made** — deliberately scoped
  ...
  `ghcr.io/compufreq/mnemosyne:<tag>-ort` amd64 image. Both
  **smoke-probed at build**: `--help` must run and
  `MNEMOSYNE_EMBEDDER=ort` without model files must fail on MODEL
...
- **A real Dockerfile bug found on the way**: the `MNEMOSYNE_FEATURES`
...
- **`LlmClient` construction now enforces TLS-or-loopback — the served
  ...
  `MNEMOSYNE_LLM_URL` was the one path still allowed to put that on a
...
- **`MNEMOSYNE_LLM_CA` (72nd env var) pins a self-signed root** for the
  LLM connection — the `MNEMOSYNE_EMBED_CA` semantics exactly:
...
- **The optional tier-2 classifier ships, advisory-only by
  construction** (`MNEMOSYNE_ADMISSION_LLM=advisory`, 71st env var; the
  model is the existing `MNEMOSYNE_LLM_*` runtime). Three properties
  ...
  - **failure is a non-event** — transport errors and answers outside
    ...
    never block a write.
- **Wired like the reranker**: a `mnemosyne_core::admission::
  AdmissionAdvisor` trait the store consults inside `admission_divert`,
  an `LlmAdmissionAdvisor` in `mnemosyne-llm` (hardened data-marked
  ...
  `MNEMOSYNE_LLM_CA` does not exist yet (a self-signed TLS LLM endpoint
...
- **`keyed_sample_capped` now bounds TWO groupings**, each at
  `want / MNEMOSYNE_TRAIN_SOURCE_CAP`: the wing (the adversarial bound
...
- **The recorded update-path gap closes, and it was provenance-shaped**:
  ...
  (`MNEMOSYNE_ADMIT_TRUSTED_SOURCES`), an update arriving over an
...
- **Retention policies per wing/room** (`retention.rs`): the operator
  ...
  (`mnemosyne retention set|clear|list`, `/v1` GET/POST `…/retention`),
...
- **Enforcement is an explicit act, never a side effect**: nothing is
  destroyed at open, on a timer, or during a write. `mnemosyne retention
...
- **The session-24 xlingual result replicates on a public corpus** —
  ...
  - *Config*: sealed vault, bge-m3 served through the shipped
    `embeddings-tls` terminator (`MNEMOSYNE_EMBEDDER=http`,
    `MNEMOSYNE_EMBED_URL=https://embeddings-tls`, `MNEMOSYNE_EMBED_CA`
  ...
  - **The full per-pair record — the ranges alone are not the record**
    ...
    `MNEMOSYNE_FUSION_WEIGHT=0.70`; the two arms differ in that one
    variable only):
  ...
  - **The honest capability claim is two-conditioned**: cross-lingual
    ...
    `MNEMOSYNE_FUSION_WEIGHT=0.70`. The default weight still does not
...
- **Cleartext http to a non-loopback embeddings host is REFUSED at
  ...
  embedder path: a deployment pointing `MNEMOSYNE_EMBED_URL` at plain
  ...
  harness rides it.
- **`MNEMOSYNE_EMBED_CA` (70th env var) declares a self-signed root as a
...
- **The required TLS infra ships containerized**: compose service
  ...
  `mnemosyne-embed-tls` volume, clients mount it read-only and pin via
  `MNEMOSYNE_EMBED_CA`. The refusal is paired with a one-command way to
...
- **`mnemosyne forget <ids> [--out att.json] [--sign key]`** destroys
...
- **Two verification postures, stated honestly.** The chain step is
  keyed, so: `mnemosyne verify-forgetting att.json` replays the segment
...
- **The provenance-driven admission posture, doctrine-clean**
  (`MNEMOSYNE_ADMIT_TRUSTED_SOURCES`, comma list, default empty — 69th
...
- **The deterministic tier-1 detector** (`mnemosyne_core::admission`):
...
- **Quarantine, not rejection** (`MNEMOSYNE_ADMISSION=quarantine`,
...
- **`keyed_sample_capped` — C3.3's density channel bounded where it
  ...
  `1/MNEMOSYNE_TRAIN_SOURCE_CAP` of the sample (default 4; `off` =
...
- **The fix: `Embedder::semantic_floor`** — the raw cosine the embedder
  ...
  `MNEMOSYNE_SEMANTIC_FLOOR` (66th env var) declares a measured floor for
...
- **Both gates, measured**:
  ...
  - *xlingual mixed-corpus recovery* (bge-m3 served, 155 pairs, sealed,
    ...
    and with a deployment-declared `MNEMOSYNE_FUSION_WEIGHT=0.70`,
...
- **Deployment-assigned wing trust classes** (C3.3, building on
  ...
  (`mnemosyne_core::TRUST_VOCAB`), assigned per wing by the RECEIVING
  PRINCIPAL — CLI `mnemosyne trust set|list` and `/v1` `POST/GET …/trust`
...
- **The floor is a candidate-set decision, never a score.**
  `SearchOptions.min_trust` (per request; `/v1`, MCP `mnemosyne_search`,
  CLI `--min-trust`) and `MNEMOSYNE_TRUST_FLOOR` (vault-level, resolved
...
- **First real `xlingual` run** (2026-08-03): bge-m3 (F16 GGUF) served
  CPU-only on the compose Ollama via `MNEMOSYNE_EMBEDDER=http`, sealed
...
- **OPEN DEFECT FILED: same-language lexical noise crowds out
  ...
  - `MNEMOSYNE_SEMANTIC_GATE=0.05` changes nothing — golds are admitted,
  ...
  - the cause is the **absolute cosine map `(cos+1)/2`**, calibrated to
    ...
    words with the query out-scores it. `MNEMOSYNE_FUSION_WEIGHT=0.70`
...
- **The user-visible price of a scoped query drops ~2.7× (wing 85 → ~32
  ...
  (`MNEMOSYNE_SEARCH_TRACE=1`, stderr, per-phase ms) then found the real
...
- **`kind` on drawers** (consultation adopted item 4, pulled forward on
  ...
  from the closed vocabulary `mnemosyne_core::KIND_VOCAB`
...
- **`SearchOptions.kind`** filters by declared kind and rides the
  ...
  save + search (unknown kind = 400), MCP (`mnemosyne_save`,
  `mnemosyne_add_drawer`, `mnemosyne_search`), CLI `search --kind`.
...
- **`mnemosyne-bench tagvalue` — the value instrument, run before any
...
- **`MNEMOSYNE_FUSION_WEIGHT` (default 0.55)** — the convex blend's
...
- **The first weight sweep ran** (LoCoMo merged corpus, hash embedder,
  no reranker, `MNEMOSYNE_RETRIEVAL` unset, harness-default 60-hit pool,
...
- **`mnemosyne-bench tagcost`** — the measurement behind the labeling
...
- **`mnemosyne-bench scopescale`** — the instrument the per-wing tier's
...
- **The first full scopescale run (sealed, hash, `retrieval=pq`, shipped
  ...
  **the scope filter earns its at-scale claim**, and the room numbers are
  ...
  nothing (91.7% — not the probe subset); `MNEMOSYNE_POOL_DIV=8` lifts
...
- **`mnemosyne-bench xlingual`** — the metric for the one capability the
  ...
  served multilingual model (`MNEMOSYNE_EMBEDDER=http`) is the
...
- **Surfaces**: store (`kg_set_authority`, `lookup_canonical`), `/v1`
  ...
  `POST /v1/vaults/{id}/kg/authority`), MCP (`mnemosyne_lookup_canonical`
  ...
  `mnemosyne_kg_set_authority`, registered as a write tool), CLI
  (`mnemosyne kg authority`, `mnemosyne kg canonical`). Key rotation
...
- **Pinned by three new starvation tests with raw premises** — each
  ...
  `MNEMOSYNE_WING_PQ_MIN=off` opts out of per-wing build cost, no longer
  out of correctness.
...
- **`Fusion::Rrf` is deleted** (`rrf_fuse`, its two rank helpers, `RRF_K`,
  ...
  **−7.3pp** turn all-gold against the BM25 blend (ROADMAP's failed table,
  ...
  `MNEMOSYNE_FUSION=rrf` now **warns and falls back to `bm25`** — a removed
...
- **Per-wing PQ indexes (`MNEMOSYNE_WING_PQ_MIN`, default 4096).** `wing` was
...
- **The settling number, measured (`pqscale`): the query-latency claim is
  ...
  **Shipped fix, arrived at in three measured steps** (each intermediate
  ...
  3. **The shipped design — a two-stage pool**: stage 1 fetches
     `live/64` ADC candidates (`MNEMOSYNE_POOL_DIV`, `off` = fixed
...
- **Every surface states the continuation instead of assuming the caller
  ...
  400, never a silent fall-back to the host clock). `mnemosyne_search` gains
...
- **Measured: the largest delivered gain in the reach program.** LoCoMo,
  default config (hash embedder, no reranker, `MNEMOSYNE_RETRIEVAL` unset,
...
- **`MNEMOSYNE_EMBEDDER=http` — an `Embedder` backed by a served model.** The
  ...
  `mnemosyne_llm::HttpEmbedder` closes that, reusing the LLM client's
  ...
  `MNEMOSYNE_EMBED_URL`/`_MODEL`/`_API`/`_KEY`/`_DIM`, and the same default-off
...
- **Late interaction gets its own depth, `MNEMOSYNE_LATE_TOP_N` (200).** It
  shared `MNEMOSYNE_RERANK_TOP_N` (50) with the cross-encoder, and the two
...
- Setting only `MNEMOSYNE_RERANK_TOP_N` still drives both stages, so a
...
- **The training sample of every trained index artifact is now a stratified
  ...
  **token rows** — a different unit and 4× the figure.
  ...
  - **The correctness reason it had to change, measured.** A fixed interval
    ...
    `MNEMOSYNE_RETRIEVAL=pq`, hmac-only, 2,000 queries:
  ...
  - **A recorded figure, and what it took to account for it.** That same
    ...
    - **Rescore depth.** It was governed by `MNEMOSYNE_RERANK_TOP_N`, an
...
- **Every codebook write bumps a visible generation counter.** Nothing in a
  ...
  - Visible on `PalaceStats.codebooks`, `GET /v1/vaults/{id}/stats` (the
    ...
    it to the struct was not enough), the MCP `mnemosyne_status` tool,
    `mnemosyne stats`, and as a **registered** telemetry gauge:
    `mnemosyne_obs::GAUGE_NAMES` is an allowlist and a gauge set under any
...
- **`remember` no longer derives a drawer id from `count()`.**
  `crates/mnemosyne-cli/src/main.rs` now uses `next_append_index()`, matching
...
- **`docs/AMB_REPLICATION.md`** — a procedure for running the Agent Memory
  ...
  rules) against Mnemosyne, with Claude subagents filling the two model roles
...
- `MNEMOSYNE_SEMANTIC_GATE` overrides whatever the embedder says — a number in
...
- **`content_date`** — when the content happened, as distinct from
  ...
  `remember --content-date`, MCP `mnemosyne_save` / `mnemosyne_add_drawer`
...
- **`MNEMOSYNE_LLM_KEY`** — optional bearer for the LLM runtime, unset by
...
- **Event-driven migration, both directions**: flipping
  `MNEMOSYNE_PQ_PAGE_MIN` (or `set_pq_pages`) repacks per-row ⇄ pages
...
- **Default off** (`MNEMOSYNE_PQ_PAGE_MIN` unset ⇒ never): per the
...
- `mnemosyne-orchestrator serve --read-replica`: opens the state
...
- **Shipped opt-in, default off** — the honest result of its own gate:
  ...
  `MNEMOSYNE_FDE_IVF_MIN=<n>` (+ `MNEMOSYNE_FDE_NPROBE`) after
...
- **PALACE tab**: the pixel-art Palace Monitor embedded in the console
  ...
  broken frame).
- **GRAFANA tab**: embed the "Mnemosyne — Palace" dashboard from the
...
- **Prebuilt release binaries**: every version tag now builds and
  attaches `mnemosyne` + `mnemosyne-orchestrator` archives for Linux
  ...
  zero telemetry deps).
- **Published container image**: `ghcr.io/compufreq/mnemosyne:<tag>` and
...
Mnemosyne is now **source-available under BUSL 1.1** (the
...
- **The one restriction**: offering Mnemosyne itself to third parties as
...
- **Heritage**: Mnemosyne remains a from-scratch Rust implementation of
...
- **`docs/AGENTS.md`** — a scenario-driven implementation guide written
  ...
  every `MNEMOSYNE_*` variable with defaults — and a verification
...
- **Landing page**: six use-case cards ("what to build with it"), a
  ...
  the four suites).
- **`MNEMOSYNE_OTLP_HEADERS` implemented** (was documented but not read
...
The second ecosystem item: `mnemosyne export --to <recipient>` seals the
...
- **`export --to <recipient> --out <file>`**: age-style construction —
  ...
  `mnemosyne.v1/bundle`), payload sealed XChaCha20-Poly1305 with the
...
`mnemosyne vault rotate <name>`: move a vault onto fresh derived keys
...
this release wires them into the `mnemosyne` binary for real
deployments.

- **New `ort` cargo feature on `mnemosyne-cli`** (opt-in, like `onnx`):
  links `mnemosyne-embed-ort` and exposes the backend at runtime —
  - `MNEMOSYNE_EMBEDDER=ort` — session-pool sentence embedder;
  - `MNEMOSYNE_RERANKER=ort` — cross-encoder scoring the whole pool in
    one batched forward (`score_batch` forwarded end to end);
  - `MNEMOSYNE_RERANKER=colbert-ort` — the ColBERT late-interaction
    encoder (search + `repair --tokens` backfill).
  Same user-supplied model files and `MNEMOSYNE_ONNX_*` / `RERANK_*` /
...
- `ort-build` compose service now compile-checks the full CLI with
  ...
  backend crate alone.
- Unknown-value errors for `MNEMOSYNE_EMBEDDER` / `MNEMOSYNE_RERANKER`
...
- **Per-tenant rate limiting** (`MNEMOSYNE_ORCH_RATE_LIMIT`,
...
instances, shipped as the **separate optional `mnemosyne-orchestrator`
...
- **State** (own SQLite): instance registry + tenant→vault map. Engine
  ...
  `MNEMOSYNE_ORCH_KEY`, AAD-bound to the instance name — a blob copied
...
- **Admin plane** `/admin/*` (`MNEMOSYNE_ORCH_ADMIN_TOKEN`, uniform
...
exactly like the token store. Raw f32 (v1) below `MNEMOSYNE_FDE_PQ_MIN`
...
- **`mnemosyne-core/src/fde.rs`**: seed-deterministic, dependency-free
...
- **`mnemosyne-store/src/fdeidx.rs`** (`MNEMOSYNE_RETRIEVAL=fde`):
...
- **Measured, end-to-end** (LoCoMo full 1,982 QA, ort colbert + tok-PQ
  ...
  cost.
- **Measured, mechanics at scale** (`mnemosyne-bench fde-synth`, exact
...
`[Q]`/`[D]`/`[MASK]` framing, same `MNEMOSYNE_COLBERT_*` env as the tract
...
- **v2 pack format**: per-token PQ codes (`pq.rs` re-used at `m=16`). The
  ...
  they cross `MNEMOSYNE_TOK_PQ_MIN` (default 256; `off` keeps int8),
