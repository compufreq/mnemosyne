...
[threat-model whitepaper](https://compufreq.github.io/mnemosyne/docs/threat-model.html)
...
- Reaching stored content **without passing the admission screen** when
  a deployment declares `MNEMOSYNE_ADMISSION=quarantine`: any write path
...
- Writes a `--read-only` process performs at **open** — schema creation,
  ...
  have you find them: under `MNEMOSYNE_RETRIEVAL=pq` a read-only search
