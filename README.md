...
- **Screened writes, receipted deletions** — opt-in admission control
  ...
  (`MNEMOSYNE_READ_AUDIT=chain` — a keyed query fingerprint, never the
...
[docs/agents.html](https://compufreq.github.io/mnemosyne/docs/agents.html).
...
- **`http`** — a model served by Ollama, llama.cpp server, LM Studio, vLLM
  or TEI (`MNEMOSYNE_EMBEDDER=http` + `MNEMOSYNE_EMBED_URL`): no export, no
  ...
  `MNEMOSYNE_EMBED_CA` pins a self-signed root (the compose
...
mnemosyne kg authority|canonical|receipts  # golden-values tier + its receipts
...
crates/
  ...
  mnemosyne-store/   SQLite per-vault storage, hybrid search, PQ/IVF + FDE
                     index tiers, admission control, forgetting, retention
  mnemosyne-cli/     `mnemosyne` binary: CLI, MCP stdio, HTTP + /v1, admin UI
  mnemosyne-index/   remote vector backends as untrusted accelerators
  mnemosyne-llm/     local LLM runtimes + the HTTP-served embedder
  mnemosyne-obs/     observability shim: no-op and zero-dep by default
  mnemosyne-orchestrator/  optional multi-tenant control plane (own binary)
  mnemosyne-bench/   retrieval benchmark + synthetic-instrument harnesses
  mnemosyne-embed-onnx/, mnemosyne-embed-ort/
