...
- `Cargo.toml` — workspace root (12 crates; `mnemosyne-embed-onnx` and
...
- `crates/mnemosyne-store` — per-vault SQLite storage, hybrid search (cosine +
  ...
  rather than by code. `MNEMOSYNE_SEMANTIC_GATE` declares it (`off` = lexical
  ...
  **standalone** fts5 table over `search_key(content)`, rebuilt on a
  ...
  `MNEMOSYNE_PQ_PAGE_MIN`, one AEAD page per list + lazy per-probe
  ...
  rule**: stage 1 fetches `live/64` ADC candidates (`MNEMOSYNE_POOL_DIV`,
  ...
  **hydration is the only door through which BM25 evidence reaches
  ...
  opt-in `MNEMOSYNE_SEARCH_TRACE=1` phase trace AFTER parallel
  ...
  Wings past `MNEMOSYNE_WING_PQ_MIN` (4096, `off` = no per-wing indexes;
  ...
  **flat across 8× corpus growth**, which is the property scoping is for —
  ...
  `MNEMOSYNE_FDE_IVF_MIN` — slab-grouped cache + sealed centroids, kept
  ...
  **`upsert_many` is the stated exception**: a batch owns its transaction,
  ...
  and the declared per-writer rate screen `MNEMOSYNE_ADMISSION_RATE=
  ...
  "corrupt row"; `MNEMOSYNE_ADMISSION=quarantine` diverts flagged
  ...
  **every read that returns content** and not from `search` alone:
  ...
  optional tier-2 advisor** (`MNEMOSYNE_ADMISSION_LLM=advisory` on the
  `MNEMOSYNE_LLM_*` runtime, `AdmissionAdvisor` trait wired like the
  ...
  candidate-set floor (`min_trust`/`MNEMOSYNE_TRUST_FLOOR`) through the
  ...
  **exports chain-audited unconditionally on every surface** —
  ...
  and serve; **reads audited under `MNEMOSYNE_READ_AUDIT=chain`** —
  ...
  `mnemosyne vault anchor` and `POST /v1/…/anchor` (an ops route on the
...
- `crates/mnemosyne-net` — the outbound transport policy, in ONE place:
  **TLS or loopback, nothing else, no override** (refused at construction,
  ...
  implemented once in `mnemosyne-llm` for the embedder and LLM clients
...
- `crates/mnemosyne-llm` — local LLM runtimes (Ollama/OpenAI-compatible) for
  ...
  backed by a served model (`MNEMOSYNE_EMBEDDER=http` + `MNEMOSYNE_EMBED_URL`
  ...
  `MNEMOSYNE_EMBED_CA` declares a self-signed root as a **pin** (replaces
  ...
  its CA on the `mnemosyne-embed-tls` volume. Two hazards it still states
  ...
  `MNEMOSYNE_LLM_CA` pins a self-signed root, construction is fallible.
...
- `crates/mnemosyne-embed-ort` — opt-in ONNX Runtime backend (C++ dep;
  ...
  CLI via `--features ort` (`MNEMOSYNE_EMBEDDER=ort`,
  `MNEMOSYNE_RERANKER=ort|colbert-ort`; multi-tenant server shares one
  session pool across vaults)
- `crates/mnemosyne-cli` — `mnemosyne` binary (main.rs: CLI, plus `Posture`
  ...
  a read-audit record per `/mcp` search; mcp.rs: MCP stdio — **34 tools (incl. `mnemosyne_history`, the audit chain
  ...
  (`MNEMOSYNE_LANG` → `LANG`, primary subtag; errors, help and
  machine-oriented output stay English);
  refine.rs: the ONE LLM-distillation implementation both `mnemosyne refine`
  and `POST /v1/…/refine` drive — same `MNEMOSYNE_LLM_*` config used to
...
- `architecture/` — illustrated architecture reference: eleven theme-aware
  ...
  `MNEMOSYNE_*` variables the engine honours — 62 written out in full
  ...
  `grep -rhoE '"MNEMOSYNE_[A-Z0-9_]+"' crates/ | sort -u` over every
  crate except `mnemosyne-bench`, whose `MNEMOSYNE_VS_*`/`MNEMOSYNE_TEST_*`
...
- `docs/AGENTS.md` — the scenario-driven agent implementation guide
  ...
  kept in sync when the MCP surface, `/v1` routes, or `MNEMOSYNE_*`
  variables change
...
- `.github/workflows/release.yml` — on every `v*` tag: five binary
  ...
  sha256, and the multi-arch `ghcr.io/compufreq/mnemosyne` image
  ...
  `MNEMOSYNE_EMBEDDER=ort` must fail on
...
#   mnemosyne-embed-tls mounts a fresh empty volume silently):
#     -v mnemosyne_mnemosyne-embed-tls:/tls:ro
#     MNEMOSYNE_EMBEDDER=http MNEMOSYNE_EMBED_URL=https://embeddings-tls
#     MNEMOSYNE_EMBED_CA=/tls/caddy/pki/authorities/local/root.crt
#     MNEMOSYNE_EMBED_MODEL=nomic-embed-text
```

**The `mnemosyne-target` volume can serve a STALE artifact.** `cargo` reused a
release rlib of `mnemosyne-vault` that predated a new method, and the resulting
...
`mnemosyne-target` volume: a build "finished in 5.18s" replaying *another
...
- Cross-lingual retrieval is the **embedder's** job and the default cannot do
  ...
  *below* an unrelated sentence, and `car`/`automobile` do not match either.
  ...
  same calibration, `MNEMOSYNE_SEMANTIC_FLOOR` declares it for external
  vaults. Gates: LoCoMo hash digit-for-digit; mixed R@5 0–4%→53–88% at
  the default weight, 100.0% every pair under a declared
  `MNEMOSYNE_FUSION_WEIGHT=0.70` — map and weight compose, the default
...
- **Independent per-item scoring is a poison-resistance property, and
  ...
  the same rule `mnemosyne-index` applies to remote backends. Coupling in
  ...
  **density** channel (owning fraction *f* bought ≈*f* of any uniform
  ...
  bounds any wing at `1/MNEMOSYNE_TRAIN_SOURCE_CAP` (default 4) of every
  ...
  **token rows** for the token codebook. **Above a cap the sample's size and
  ...
  and a gauge whose name must be in `mnemosyne_obs::GAUGE_NAMES` or it is
...
- **Docs vs code**: every number, tool table, route table and `MNEMOSYNE_*`
...
- **Drift check before every release**, not only when something feels off.
  ...
  wiring incl. every `MNEMOSYNE_*` variable per surface, write path, search
