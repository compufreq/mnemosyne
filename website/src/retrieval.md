...
Serve one with `MNEMOSYNE_EMBEDDER=http` + `MNEMOSYNE_EMBED_URL`. The
...
`MNEMOSYNE_EMBED_CA` pins a self-signed root (a garbage file refuses rather
...
A research spike (`mnemosyne-bench pqpage-synth`) priced the multi-million
...
default, and the **sealed page tier ships behind `MNEMOSYNE_PQ_PAGE_MIN`** —
...
They no longer do. `mnemosyne-bench pqscale` and `scopescale` grow one
...
- **The hotspot was not where anyone thought.** Parallel candidate hydration —
  ...
  (`MNEMOSYNE_SEARCH_TRACE=1`) then found the cost in BM25's serial
...
build with `--features ort`, then `MNEMOSYNE_EMBEDDER=ort` /
`MNEMOSYNE_RERANKER=ort` / `MNEMOSYNE_RERANKER=colbert-ort` select it at
...
**MUVERA FDE candidates** (`MNEMOSYNE_RETRIEVAL=fde`) extend token-awareness
...
| **MUVERA FDE** (`MNEMOSYNE_RETRIEVAL=fde`) | ~O(codebook) | token-aware candidates |
