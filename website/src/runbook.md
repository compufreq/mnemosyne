...
2. **Freeze writes.** Restart the server read-only so nothing new is written on
   ...
   `MNEMOSYNE_RETRIEVAL=pq` a search may still build a missing PQ/IVF index,
...
1. **Restore from the most recent good backup.** `backup` refuses to run if the
   ...
   ```bash
   mnemosyne backup list                          # names are <vault>-<stamp>
   mnemosyne backup restore <vault>-<stamp> --force   # --force to overwrite the live vault
   mnemosyne verify --vault <vault>   # must now report 0 hmac failures, chain ok
   ```
...
3. **Housekeeping** after a clean restore:
   ```bash
   mnemosyne repair --vault <vault>  # backfill fingerprints, vacuum, re-verify
   ```
...
- **Back up on a schedule.** `mnemosyne backup create --vault <vault>` is the
