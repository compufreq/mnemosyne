...
if "$ORCH" --db "$MNEMOSYNE_ORCH_DB" ops "$OPS_ID" verify 2>&1 | grep -q '"ok":true'; then
  ok "orchestrator CLI ops verify"
else
  fail "orchestrator CLI ops verify"
fi
if "$ORCH" --db "$MNEMOSYNE_ORCH_DB" ops "$OPS_ID" trust 2>&1 | grep -q 'assignments'; then
...
OPS_ERR="$("$ORCH" --db "$MNEMOSYNE_ORCH_DB" ops "$OPS_ID" frobnicate 2>&1)"
