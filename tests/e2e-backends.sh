...
if [ -s "$CADDY_ROOT" ] && [ -s /pgtls/ca.crt ]; then
  cat "$CADDY_ROOT" /pgtls/ca.crt > "$MNEMOSYNE_INDEX_CA"
  echo "ok    pinned both roots into $MNEMOSYNE_INDEX_CA"; PASS=$((PASS+1))
else
...
run_backend_suite() { # run_backend_suite <backend>
  ...
  check "[$be] refuses cleartext" 1 "no override" -- env     MNEMOSYNE_QDRANT_URL=http://qdrant:6333     MNEMOSYNE_CHROMA_URL=http://chroma:8000     MNEMOSYNE_MILVUS_URL=http://milvus:19530     MNEMOSYNE_WEAVIATE_URL=http://weaviate:8080     MNEMOSYNE_PGVECTOR_DSN=postgresql://mnemosyne:mnemosyne@pgvector:5432/mnemosyne     "$BIN" index push "$be"
...
: > /tmp/empty-ca.pem
check "empty CA pin refuses" 1 "pins nothing" -- env MNEMOSYNE_INDEX_CA=/tmp/empty-ca.pem   "$BIN" index status qdrant
