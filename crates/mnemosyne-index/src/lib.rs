...
//!   `MNEMOSYNE_INDEX_CA` pinning a self-signed root — because an embedding
...
/// `MNEMOSYNE_EMBED_CA` and `MNEMOSYNE_LLM_CA` do. A garbage file refuses to
/// open rather than falling back — un-pinning silently is the failure mode.
pub const CA_VAR: &str = "MNEMOSYNE_INDEX_CA";
...
pub(crate) fn backend_agent_with(
    ...
    mnemosyne_net::agent("the remote index", base_url, ca.as_deref(), timeout)
...
pub fn from_env(backend: &str) -> Result<Box<dyn VectorIndex>, IndexError> {
    match backend {
        ...
        "weaviate" => {
            let url = std::env::var("MNEMOSYNE_WEAVIATE_URL")
                .map_err(|_| IndexError::NotConfigured("weaviate", "MNEMOSYNE_WEAVIATE_URL"))?;
...
pub mod pgvector {
    ...
    pub(crate) fn dsn_is_loopback(dsn: &str) -> bool {
        ...
        if d.starts_with("postgres://") || d.starts_with("postgresql://") {
            return mnemosyne_net::is_loopback(d);
        }
        ...
        pub fn new(dsn: &str) -> Result<Self, IndexError> {
            ...
            if !dsn_is_loopback(dsn) && !dsn_demands_tls(dsn) {
                return Err(IndexError::Transport(format!(
                    ...
                    "MNEMOSYNE_PGVECTOR_DSN", CA_VAR
                )));
            }
            let client = if dsn_demands_tls(dsn) {
                let cfg = match std::env::var(CA_VAR) {
                    Ok(path) => mnemosyne_net::pinned_roots_from_file("the remote index", &path)
                        .map_err(|e| IndexError::Transport(e.to_string()))?,
                    Err(_) => mnemosyne_net::webpki_roots(),
                };
