...
fn resolve_semantic_gate<E: Embedder + ?Sized>(embedder: &E) -> Option<f32> {
    match std::env::var("MNEMOSYNE_SEMANTIC_GATE") {
...
/// per search (override with `MNEMOSYNE_RERANK_TOP_N`). One transformer
...
/// both stages shared `MNEMOSYNE_RERANK_TOP_N`, late interaction inherited a
...
///
/// Falls back to `MNEMOSYNE_RERANK_TOP_N` when only that is set, so an
/// operator who pinned the old single knob keeps the behaviour they pinned;
/// `MNEMOSYNE_LATE_TOP_N` wins when both are set.
///
/// **The fallback covers unparseable values too**, and that is not
/// pedantry: `MNEMOSYNE_RERANK_TOP_N=0` (or `abc`, or empty) has always meant
...
/// changed nothing. So a *present* `MNEMOSYNE_RERANK_TOP_N` pins this stage to
/// whatever the reranker resolves it to, valid or not.
pub(crate) fn late_top_n() -> usize {
    resolve_late_top_n(
        std::env::var("MNEMOSYNE_LATE_TOP_N").ok().as_deref(),
        std::env::var("MNEMOSYNE_RERANK_TOP_N").ok().as_deref(),
    )
}
...
/// The convex blend's semantic weight (`MNEMOSYNE_FUSION_WEIGHT`):
...
/// `MNEMOSYNE_SEMANTIC_FLOOR` declares it (a raw cosine in `[0.0, 0.98]`;
...
fn resolve_semantic_floor<E: Embedder + ?Sized>(embedder: &E) -> f32 {
    match std::env::var("MNEMOSYNE_SEMANTIC_FLOOR") {
        ...
        Ok(v) => match v.trim().parse::<f32>() {
            ...
            _ => {
                mnemosyne_obs::diag_warn!(
                    "MNEMOSYNE_SEMANTIC_FLOOR={v:?} is not a cosine in [0.0, 0.98]; \
...
fn resolve_trust_floor(env: Option<&str>) -> Option<String> {
    ...
    match mnemosyne_core::validate_trust(&v) {
        Ok(()) => Some(v),
        Err(_) => {
            mnemosyne_obs::diag_warn!(
                "MNEMOSYNE_TRUST_FLOOR={v:?} is not in the trust vocabulary \
...
/// `MNEMOSYNE_READ_AUDIT`: `chain` puts a record on the audit chain for
...
fn resolve_read_audit(env: Option<&str>) -> Result<bool, StoreError> {
    match env.map(str::trim) {
        ...
        Some(v) => Err(StoreError::Invalid(format!(
            "MNEMOSYNE_READ_AUDIT={v:?} — the only modes are 'chain' and 'off'; \
             refusing to open with an unreadable audit declaration"
        ))),
    }
}

/// The declared per-writer admission rate (`MNEMOSYNE_ADMISSION_RATE`,
...
fn resolve_admission_rate(env: Option<&str>) -> Result<Option<(u32, u32)>, StoreError> {
    ...
    parsed.map(Some).ok_or_else(|| {
        StoreError::Invalid(format!(
            "MNEMOSYNE_ADMISSION_RATE={v:?} — expected <count>/<seconds> with both \
...
fn resolve_fusion_weight(env: Option<&str>) -> f32 {
    match env {
        ...
        Some(v) => match v.trim().parse::<f32>() {
            ...
            _ => {
                mnemosyne_obs::diag_warn!(
                    "MNEMOSYNE_FUSION_WEIGHT={v:?} is not a number; \
...
fn resolve_late_top_n(late: Option<&str>, rerank: Option<&str>) -> usize {
    ...
    match rerank {
        ...
        // that do not parse. `MNEMOSYNE_RERANK_TOP_N=0` has always resolved to
...
///
/// Override at open with `MNEMOSYNE_FUSION` (`bm25` / `legacy`).
    ...
    fn from_env() -> Self {
        match std::env::var("MNEMOSYNE_FUSION").ok().as_deref() {
            ...
            Some(v) if v.eq_ignore_ascii_case("rrf") => {
                mnemosyne_obs::diag_warn!(
                    "MNEMOSYNE_FUSION=rrf was removed (measured −7.3pp vs bm25); \
...
pub enum StoreError {
    ...
    /// wore it, `mnemosyne verify-forgetting` exited 1 on a forged
    ...
    #[error(
        "this vault's schema predates this build ({missing}) and a read-only open must not \
         ...
         command, or `mnemosyne verify`) to migrate, then retry read-only."
    )]
...
/// deployment declaring `MNEMOSYNE_ADMIT_TRUSTED_SOURCES` can name the
...
/// Whether `id` has the shape [`mnemosyne_core::ids::drawer_id`] produces:
...
/// every drawer in the batch, so `mnemosyne import` printed "imported
...
pub struct SearchOptions {
    ...
    /// [`mnemosyne_core::KIND_VOCAB`]; an unknown value is an error, never
    ...
    /// [`mnemosyne_core::TRUST_VOCAB`]; unknown = an error, never an empty
    ...
    /// Composes with the vault-level `MNEMOSYNE_TRUST_FLOOR`; an explicit
...
pub struct PalaceStore {
    ...
    fusion_weight: f32,
    /// The vault-level trust floor (`MNEMOSYNE_TRUST_FLOOR`), resolved once
    ...
    /// (`MNEMOSYNE_ADMISSION=quarantine`; default off — admission changes
    ...
    admission_advisor: Option<Box<dyn mnemosyne_core::admission::AdmissionAdvisor + Send + Sync>>,
    /// The declared per-writer rate screen (`MNEMOSYNE_ADMISSION_RATE`,
    ...
    /// Chain-audit reads (`MNEMOSYNE_READ_AUDIT=chain`; unset = off — a
    ...
    /// (`MNEMOSYNE_ADMIT_TRUSTED_SOURCES`, comma list matched against the
    ...
    /// (`MNEMOSYNE_TRAIN_SOURCE_CAP`, default 4 = no wing supplies more
    ...
    /// `MNEMOSYNE_POOL_DIV` (number, `off`) / [`Self::set_pool_div`].
    pool_div: usize,
    ...
    /// `MNEMOSYNE_LATE_TOP_N` / `MNEMOSYNE_RERANK_TOP_N`
    ...
    /// `MNEMOSYNE_PQ_PAGE_MIN` / [`PalaceStore::set_pq_pages`]. See `pqidx`.
    pq_page_min: usize,
    /// When true, search generates candidates by MUVERA FDE dot product
    /// (`MNEMOSYNE_RETRIEVAL=fde`; needs the late encoder). See `fdeidx`.
    fde_enabled: bool,
    /// The deterministic FDE encoder (params persisted sealed in `fde_meta`).
    fde_encoder: std::cell::RefCell<Option<mnemosyne_core::fde::FdeEncoder>>,
    ...
    /// (`MNEMOSYNE_FDE_PQ_MIN`, `off` ⇒ never — raw v1 rows only).
    fde_pq_min: usize,
    ...
    /// (`MNEMOSYNE_FDE_IVF_MIN`, `off` ⇒ never.) Below this the flat ADC
    /// scan stays — it was the measured winner at every small-N scale.
    fde_ivf_min: usize,
    /// `MNEMOSYNE_FDE_NPROBE` (default `max(8, nlist/4)` — recall tracks
...
impl PalaceStore {
    ...
    /// `MNEMOSYNE_RETRIEVAL=pq` (or `fde`, or a late-interaction rescore) a
    ...
    pub fn open_read_only(
        ...
        if store.read_audit {
            mnemosyne_obs::diag_warn!(
                "MNEMOSYNE_READ_AUDIT=chain declared but this open is read-only; \
    ...
    fn enforce_embedder_identity(&mut self, may_migrate: bool) -> Result<(), StoreError> {
        ...
        match (stored_name, stored_dim) {
            ...
            // write — the same rule the `MNEMOSYNE_FORCE_EMBEDDER` arm above
    ...
    /// `mnemosyne_obs::GAUGE_NAMES` does not list is silently dropped, so the
    ...
    fn note_unblinded_kg(&mut self) {
        ...
        match self.unkeyed_fingerprint_rows() {
            Ok(0) => {}
            Ok(n) => self.unhealed.push(format!(
                "{n} content fingerprint(s) are still an UNKEYED SHA-256 of a cited drawer's                  verbatim content, in a clear column: the U12 migration skipped them because                  their receipt does not verify, and re-tagging a tampered binding would                  launder it. An offline reader holding a candidate document can confirm it.                  `mnemosyne verify` names them; this vault is not marked migrated, so a                  writable open retries"
            )),
            Err(_) => {}
        }
    }
    ...
    fn open_inner_read_only(
        ...
        for note in &store.unhealed {
            mnemosyne_obs::diag_warn!("read-only open left this unhealed: {note}");
        }
        Ok(store)
    }
            ...
            AnchorState::Healed { behind_by } => {
                self.unhealed.push(format!(
                    "the manifest rollback anchor is {behind_by} record(s) behind the \
                     ...
                     cause, and `mnemosyne vault anchor` or any write heals it"
                ));
                Ok(())
            }
        }
    ...
    fn reconcile_chain(&mut self, heal: bool) -> Result<AnchorState, StoreError> {
        ...
        let tags: Vec<Vec<u8>> = stmt
            ...
            .collect::<Result<_, _>>()?;
        drop(stmt);
        let genesis = mnemosyne_vault::Vault::chain_genesis_hex();
    ...
    fn assemble(
        ...
        let store = Self {
            ...
            fusion_weight: resolve_fusion_weight(
                std::env::var("MNEMOSYNE_FUSION_WEIGHT").ok().as_deref(),
            ),
            trust_floor: resolve_trust_floor(
                std::env::var("MNEMOSYNE_TRUST_FLOOR").ok().as_deref(),
            ),
            sem_floor,
            admission_quarantine: match std::env::var("MNEMOSYNE_ADMISSION") {
                Ok(v) if v.eq_ignore_ascii_case("quarantine") => true,
                Ok(v) if v.eq_ignore_ascii_case("off") || v.is_empty() => false,
                Ok(v) => {
                    mnemosyne_obs::diag_warn!(
                        "MNEMOSYNE_ADMISSION={v:?} is not 'quarantine' or 'off'; \
                         admission stays off"
                    );
                    false
                }
            ...
            admission_rate: resolve_admission_rate(
                std::env::var("MNEMOSYNE_ADMISSION_RATE").ok().as_deref(),
            )?,
            read_audit: resolve_read_audit(std::env::var("MNEMOSYNE_READ_AUDIT").ok().as_deref())?,
            admit_trusted_sources: std::env::var("MNEMOSYNE_ADMIT_TRUSTED_SOURCES")
                .map(|v| {
                    v.split(',')
                        .map(str::trim)
                        .filter(|s| !s.is_empty())
                ...
                .unwrap_or_default(),
            train_source_cap: match std::env::var("MNEMOSYNE_TRAIN_SOURCE_CAP") {
                Ok(v) if v.eq_ignore_ascii_case("off") => usize::MAX,
                Ok(v) => v.parse().ok().filter(|&d| d >= 2).unwrap_or_else(|| {
                    mnemosyne_obs::diag_warn!(
                        "MNEMOSYNE_TRAIN_SOURCE_CAP={v:?} is not an integer >= 2 or 'off'; \
                         using 4"
                    );
                    4
                }),
            ...
            wing_pq_min: match std::env::var("MNEMOSYNE_WING_PQ_MIN") {
                Ok(v) if v.eq_ignore_ascii_case("off") => usize::MAX,
                Ok(v) => v.parse().unwrap_or(pqidx::WING_PQ_MIN_DEFAULT),
                Err(_) => pqidx::WING_PQ_MIN_DEFAULT,
            },
            pool_div: match std::env::var("MNEMOSYNE_POOL_DIV") {
                Ok(v) if v.eq_ignore_ascii_case("off") => usize::MAX,
            ...
            pq_page_min: match std::env::var("MNEMOSYNE_PQ_PAGE_MIN") {
                Ok(v) if v.eq_ignore_ascii_case("off") => usize::MAX,
            ...
            fde_pq_min: match std::env::var("MNEMOSYNE_FDE_PQ_MIN") {
                Ok(v) if v.eq_ignore_ascii_case("off") => usize::MAX,
            ...
            fde_ivf_min: match std::env::var("MNEMOSYNE_FDE_IVF_MIN") {
                Ok(v) if v.eq_ignore_ascii_case("off") => usize::MAX,
                Ok(v) => v.parse().unwrap_or(fdeidx::FDE_IVF_MIN_DEFAULT),
                Err(_) => usize::MAX,
            },
            fde_nprobe: std::env::var("MNEMOSYNE_FDE_NPROBE")
                .ok()
                .and_then(|v| v.parse().ok()),
    ...
    /// `MNEMOSYNE_FUSION` at open (`legacy` / `bm25`, bm25 otherwise).
    /// See [`Fusion`].
    ...
    /// [`DEFAULT_FUSION_WEIGHT`] / `MNEMOSYNE_FUSION_WEIGHT`.
    ...
    pub fn manifest_facts(&self) -> Result<(String, String, String, String), StoreError> {
        ...
        let level = match self.vault.level() {
            mnemosyne_vault::SecurityLevel::Sealed => "sealed",
            mnemosyne_vault::SecurityLevel::HmacOnly => "hmac-only",
        };
        Ok((
    ...
    fn emit_write_event(&self, drawer: &Drawer, deduped: bool) {
        match crate::admission::save_event(drawer) {
            crate::admission::SaveEvent::Saved => {
                mnemosyne_obs::drawer_write(if deduped {
                    mnemosyne_obs::WriteOutcome::Deduped
                } else {
                    mnemosyne_obs::WriteOutcome::Created
                });
            ...
            crate::admission::SaveEvent::Quarantined {
                ...
                mnemosyne_obs::drawer_write(mnemosyne_obs::WriteOutcome::Quarantined);
                mnemosyne_obs::event_drawer_quarantined(
                    self.vault.id(),
                    intended_wing,
                    &drawer.meta.room,
                    &codes,
    ...
    fn write_drawer_stmts(
        ...
        mnemosyne_core::validate_name(&drawer.meta.wing, "wing")
            .map_err(|e| StoreError::Invalid(e.to_string()))?;
        mnemosyne_core::validate_name(&drawer.meta.room, "room")
            .map_err(|e| StoreError::Invalid(e.to_string()))?;
        ...
        // ([`mnemosyne_core::ids::drawer_id`] — 32 hex characters), never
        ...
        // `mnemosyne remember`, so the declared maximum was a property of
        // one entry point rather than of the vault.
        mnemosyne_core::validate_content_len(&drawer.content)
            .map_err(|e| StoreError::Invalid(e.to_string()))?;
        ...
        if let Some(k) = drawer.meta.kind.as_deref() {
            mnemosyne_core::validate_kind(k).map_err(|e| StoreError::Invalid(e.to_string()))?;
        }
    ...
    fn import_unwrap_screened(drawer: &Drawer) -> Result<Drawer, StoreError> {
        ...
        d.id =
            mnemosyne_core::ids::drawer_id(&d.meta.wing, &d.meta.room, source, d.meta.chunk_index);
        Ok(d)
    }
    ...
    /// out of the payload, so with `MNEMOSYNE_ADMIT_TRUSTED_SOURCES=cli`
    ...
    pub(crate) fn verified_meta_admits(
        meta: &mnemosyne_core::DrawerMeta,
    ...
    pub(crate) fn resolve_search_policy(
        ...
        if let Some(t) = opts.min_trust.as_deref() {
            mnemosyne_core::validate_trust(t).map_err(|e| StoreError::Invalid(e.to_string()))?;
        }
    ...
    fn search_inner(
        ...
        // Opt-in phase trace (`MNEMOSYNE_SEARCH_TRACE=1`): where one search
        ...
        // the next guess.
        let trace = std::env::var("MNEMOSYNE_SEARCH_TRACE").is_ok();
        ...
        let cands: Vec<Candidate> = rows
            ...
            .map(|((id, meta_json, content_rest, emb_rest, tag), cached)| {
                vault
                    ...
                    .map_err(|_| {
                        mnemosyne_obs::hmac_verify_failed("drawer");
                        mnemosyne_obs::event_hmac_fail(vault.id(), "drawer");
                        StoreError::Integrity(id.clone())
                    })?;
        ...
        let mut hits = match self.fusion {
            ...
            Fusion::Bm25 => {
                ...
                let qmask = mnemosyne_core::script::letter_script_mask(query);
                cands
                    .into_iter()
                    .zip(bm25)
                    .map(|(c, (lexical, lexical_exact, lexical_morph))| {
                        let dmask = mnemosyne_core::script::letter_script_mask(&c.drawer.content);
                        let pw = if mnemosyne_core::script::scripts_disjoint(qmask, dmask) {
                            FUSION_WEIGHT_MAX
                        } else {
                            w
                        };
    ...
    /// `MNEMOSYNE_READ_AUDIT` resolved at open is the deployment's way).
    ...
    /// (R3) — `mnemosyne vault anchor`, or `POST /v1/vaults/{id}/anchor`
    ...
    /// CLI the advice worked by accident (a fresh `mnemosyne verify`
    ...
    pub fn audit_export(
        &mut self,
        surface: &str,
        counts: &mnemosyne_vault::bundle::ManifestCounts,
        payload_sha256: &str,
    ...
    pub(crate) fn score_drawer(
        ...
        let w = if mnemosyne_core::script::scripts_disjoint(
            mnemosyne_core::script::letter_script_mask(query),
            mnemosyne_core::script::letter_script_mask(&drawer.content),
        ) {
            FUSION_WEIGHT_MAX
        } else {
            self.fusion_weight
    ...
    pub fn verify(&self) -> Result<VerifyReport, StoreError> {
        ...
        for (id, meta_json, content_rest, tag, wing, room, kind, supersedes) in rows {
            ...
            let Ok(meta) = serde_json::from_str::<mnemosyne_core::DrawerMeta>(&meta_json) else {
...
fn bm25_raw(qterms: &[String], cands: &[Candidate], lang: MorphLang) -> Bm25 {
    ...
    // its time. The phase trace (`MNEMOSYNE_SEARCH_TRACE`) measured it at
...
mod tests {
    ...
    fn fde_candidates_rank_the_target_and_seal_at_rest() {
        for level in [SecurityLevel::HmacOnly, SecurityLevel::Sealed] {
            ...
            let expected_plain: Vec<u8> = {
                let enc = mnemosyne_core::fde::FdeEncoder::new(
                    16,
                    mnemosyne_core::fde::FdeParams::default(),
                );
                let m = mnemosyne_core::late::LateInteraction::encode_doc(
                    &WordLate,
    ...
    fn one_drawer_costs_exactly_this_many_bytes() {
        ...
        const DIM: usize = mnemosyne_core::embed::EMBED_DIM; // 384
        ...
        // `MNEMOSYNE_RETRIEVAL` unset there is no PQ row and no FDE row at all —
    ...
    fn every_write_path_is_screened() {
        ...
        let vec4 = vec![0.1f32; mnemosyne_core::embed::EMBED_DIM];
    ...
    fn an_import_cannot_forge_pending_review_evidence() {
        ...
        forged.meta.admission_signals = vec![mnemosyne_core::admission::AdmissionSignal {
    ...
    ///
    /// The exploit, verbatim: `MNEMOSYNE_ADMISSION=quarantine` diverts a
    ...
    /// never appears — so both checks passed and `mnemosyne_follow_tunnel`
    ...
    /// but every delete path was open: `mnemosyne_delete_drawer` over MCP —
    ...
    fn the_admission_state_machine_converges_from_every_crash_window() {
        ...
        restored.id = mnemosyne_core::ids::drawer_id(
            &restored.meta.wing,
            &restored.meta.room,
    ...
    fn exports_leave_a_chain_record_binding_the_manifest_digest() {
        ...
        let counts = mnemosyne_vault::bundle::ManifestCounts {
            drawers: 1,
            ..Default::default()
        };
    ...
    /// `MNEMOSYNE_READ_AUDIT` parses its two modes or REFUSES to open —
    ...
    /// `MNEMOSYNE_ADMISSION_RATE` parses `<count>/<seconds>` or REFUSES
    ...
    fn forgetting_is_proven_and_the_proof_is_falsifiable() {
        ...
        let (secret, _) = mnemosyne_vault::bundle::sign_keygen();
    ...
    fn the_llm_advisor_pushes_toward_quarantine_and_never_admits() {
        ...
        impl mnemosyne_core::admission::AdmissionAdvisor for Stub {
    ...
    fn an_update_is_screened_once_and_the_reported_verdict_governs() {
        ...
        impl mnemosyne_core::admission::AdmissionAdvisor for Flipping {
            fn assess(&self, _content: &str) -> Option<bool> {
    ...
    fn the_write_choke_point_validates_names_and_the_content_bound() {
        ...
        let over = "x".repeat(mnemosyne_core::MAX_CONTENT_BYTES + 1);
        ...
        let at = "x".repeat(mnemosyne_core::MAX_CONTENT_BYTES);
    ...
    /// `MNEMOSYNE_RETRIEVAL=pq` turned the flag that promises "this process
    ...
    /// and returned a bare created-count, so `mnemosyne import` printed
    ...
    /// `MNEMOSYNE_RERANK_TOP_N=0` has always resolved to 50, because the
    ...
    fn the_two_rescore_depths_resolve_independently() {
        ...
        for pinned in ["0", "-1", "abc", "", "99999999999999999999"] {
            assert_eq!(
                ...
                "MNEMOSYNE_RERANK_TOP_N={pinned:?} must not silently deepen \
    ...
    struct SynonymLate;
    impl mnemosyne_core::late::LateInteraction for SynonymLate {
        fn model_name(&self) -> &str {
            "synonym-mock"
        }
        fn dim(&self) -> usize {
    ...
    /// A gauge set under a name `mnemosyne_obs` does not register is dropped
    ...
    fn every_codebook_gauge_name_is_registered_in_obs() {
        for artifact in CODEBOOK_ARTIFACTS {
            ...
            assert!(
                mnemosyne_obs::GAUGE_NAMES.contains(&name.as_str()),
                "gauge {name:?} (for artifact {artifact:?}) is not in \
                 mnemosyne_obs::GAUGE_NAMES, so setting it is a no-op. Add it \
    ...
    /// `MNEMOSYNE_READ_AUDIT=chain` appends one chain record per search and
    ...
    fn ro(mgr: &VaultManager, id: &str) -> Result<PalaceStore, StoreError> {
        PalaceStore::open_read_only(
            mgr.unlock_as(id, mnemosyne_vault::Access::ReadOnly)
                .unwrap(),
    ...
    fn a_read_only_open_does_not_stamp_a_fresh_vault() {
        ...
        drop(
            PalaceStore::open_read_only(
                mgr.unlock_as("test", mnemosyne_vault::Access::ReadOnly)
                    .unwrap(),
                Box::new(mnemosyne_core::HashEmbedder),
            )
            .unwrap(),
        );
        assert!(
    ...
    fn the_semantic_gate_is_calibrated_to_the_default_embedder() {
        let e = HashEmbedder;
        let ceiling = 2.0 * mnemosyne_core::embed::HASH_ADMISSION_GATE - 1.0;
        let unrelated = [
    ...
    impl Embedder for HighFloorEmbedder {
        ...
        fn dimension(&self) -> usize {
            mnemosyne_core::embed::EMBED_DIM
        }
        fn embed(&self, text: &str) -> Vec<f32> {
            let d = mnemosyne_core::embed::EMBED_DIM;
    ...
    impl Embedder for BrokenEmbedder {
        ...
        fn embed(&self, _text: &str) -> Vec<f32> {
            vec![0.0; mnemosyne_core::embed::EMBED_DIM]
        }
    }
    ...
    fn a_high_floor_space_is_recalibrated_not_out_gated() {
        ...
        let floor = mnemosyne_core::embed::calibrate_semantic_floor(&e)
        ...
        assert_eq!(
            gate,
            mnemosyne_core::embed::HASH_ADMISSION_GATE,
    ...
    fn an_external_vault_refuses_semantic_only_admission() {
        assert_eq!(
            mnemosyne_core::embed::ExternalEmbedder::new("gateway", 8).semantic_admission_gate(),
            None
        );
        let dir = TempDir::new().unwrap();
        let mgr = VaultManager::open(dir.path(), None).unwrap();
        ...
        let s = PalaceStore::open_with_embedder(
            vault,
            Box::new(mnemosyne_core::embed::ExternalEmbedder::new("gateway", 8)),
        )
        .unwrap();
    ...
    /// `MNEMOSYNE_SEMANTIC_GATE` is for an operator who has measured their
    ...
    fn an_operator_can_declare_or_disable_the_gate() {
        let _env = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        std::env::set_var("MNEMOSYNE_SEMANTIC_GATE", "0.8");
        assert_eq!(resolve_semantic_gate(&HashEmbedder), Some(0.8));
        std::env::set_var("MNEMOSYNE_SEMANTIC_GATE", "off");
        assert_eq!(resolve_semantic_gate(&HashEmbedder), None);
        for junk in ["banana", "1.5", "-0.2", ""] {
            std::env::set_var("MNEMOSYNE_SEMANTIC_GATE", junk);
            assert_eq!(
                resolve_semantic_gate(&HashEmbedder),
                Some(mnemosyne_core::embed::HASH_ADMISSION_GATE),
                "{junk:?} did not fall back to the embedder"
            );
        }
        std::env::remove_var("MNEMOSYNE_SEMANTIC_GATE");
    }
...
=== RUSSIAN (vocab {}) ===",
    ...
    fn approximate_evidence_alone_does_not_admit_a_drawer() {
        ...
        for h in &hits {
            assert!(
                h.lexical_exact > 0.0 || h.semantic > mnemosyne_core::embed::HASH_ADMISSION_GATE,
