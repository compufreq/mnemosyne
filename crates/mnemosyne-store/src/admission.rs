...
//! whole lifecycle chain-audited. See `mnemosyne_core::admission` for the
//! detector and its honest boundaries, and THREAT_MODEL.md §8 for the
//! design this implements.
//!
//! **Default OFF** (`MNEMOSYNE_ADMISSION=quarantine` opts in): admission
...
pub struct PendingAdmission {
    ...
    pub signals: Vec<mnemosyne_core::admission::AdmissionSignal>,
    pub filed_at: String,
}
...
impl PalaceStore {
    ...
    /// `MNEMOSYNE_ADMISSION` resolved at open is the deployment's way).
    ...
    /// `MNEMOSYNE_ADMIT_TRUSTED_SOURCES` resolved at open is the
    /// deployment's way).
    ...
    /// `MNEMOSYNE_ADMISSION_RATE` resolved at open is the deployment's
    ...
    /// reranker — `MNEMOSYNE_ADMISSION_LLM=advisory` resolves to this).
    ...
    pub fn set_admission_advisor(
        &mut self,
        advisor: Option<Box<dyn mnemosyne_core::admission::AdmissionAdvisor + Send + Sync>>,
    ) {
    ...
    pub(crate) fn admission_divert(&self, drawer: &Drawer) -> Option<Drawer> {
        ...
        let mut signals = mnemosyne_core::admission::screen(&drawer.content);
        ...
        if self.rate_flagged(drawer) {
            signals.push(mnemosyne_core::admission::AdmissionSignal {
                code: mnemosyne_core::admission::RATE_ANOMALY_CODE.to_string(),
                offset: 0,
            });
        }
        if signals.is_empty() {
            ...
            match self.admission_advisor.as_ref()?.assess(&drawer.content) {
                Some(true) => signals.push(mnemosyne_core::admission::AdmissionSignal {
                    code: mnemosyne_core::admission::LLM_ADVISORY_CODE.to_string(),
                    offset: 0,
                }),
        ...
        d.id = mnemosyne_core::ids::drawer_id(
            QUARANTINE_WING,
            &d.meta.room,
            source,
            d.meta.chunk_index,
    ...
    fn rate_flagged(&self, drawer: &Drawer) -> bool {
        ...
        match counted {
            ...
            Err(e) => {
                ...
                mnemosyne_obs::diag_warn!("admission rate query failed ({e}); write not screened");
                false
            }
        }
    }
