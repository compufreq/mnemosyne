...
/// `synth --n 16384` with `MNEMOSYNE_RETRIEVAL=pq` — where `⌈n/4096⌉ = 4` meets
...
/// sample. Tunable: `MNEMOSYNE_WING_PQ_MIN` (`off` disables the per-wing
...
impl PalaceStore {
    ...
    /// (the env `MNEMOSYNE_SEMANTIC_FLOOR` resolved at open is the
    ...
    /// `MNEMOSYNE_TRUST_FLOOR` resolved at open is the deployment's way).
    ...
    pub fn set_trust_floor(&mut self, floor: Option<String>) -> Result<(), StoreError> {
        if let Some(f) = floor.as_deref() {
            mnemosyne_core::validate_trust(f).map_err(|e| StoreError::Invalid(e.to_string()))?;
        }
    ...
    /// behavior exactly). Default from `MNEMOSYNE_WING_PQ_MIN` at open
    /// (`off` ⇒ never).
    ...
    /// behavior). Default from `MNEMOSYNE_POOL_DIV` at open (`off` ⇒
    ...
    pub(crate) fn wing_pq_candidates_in(
        ...
        mnemosyne_obs::search_wings_probed(1);
        Ok(Some(scored.into_iter().map(|(_, seq)| seq).collect()))
    }
