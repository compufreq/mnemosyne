...
impl ForgetAttestation {
    ...
    pub fn sign(&mut self, signing_secret_hex: &str) -> Result<(), StoreError> {
        self.sender = Some(
            mnemosyne_vault::bundle::signer_of(signing_secret_hex)
                .map_err(|e| StoreError::Invalid(e.to_string()))?,
        );
        self.sig = Some(
            mnemosyne_vault::bundle::sign_detached(signing_secret_hex, &self.canonical())
                .map_err(|e| StoreError::Invalid(e.to_string()))?,
        );
        Ok(())
    }
...
impl PalaceStore {
    ...
    pub fn verify_forget_attestation(&self, att: &ForgetAttestation) -> Result<(), StoreError> {
        ...
        if let (Some(sender), Some(sig)) = (att.sender.as_deref(), att.sig.as_deref()) {
            mnemosyne_vault::bundle::verify_detached(sender, &att.canonical(), sig)
