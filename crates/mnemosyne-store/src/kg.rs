...
impl PalaceStore {
    ...
    pub fn kg_import(&mut self, exp: &TripleExport) -> Result<String, StoreError> {
        let t = &exp.triple;
        mnemosyne_core::validate_name(&t.subject, "subject").map_err(|e| {
            StoreError::CorruptRow {
                id: t.subject.clone(),
                reason: e.to_string(),
            }
        })?;
        mnemosyne_core::validate_name(&t.predicate, "predicate").map_err(|e| {
            StoreError::CorruptRow {
    ...
    pub fn kg_import_entity(&mut self, name: &str, etype: &str) -> Result<(), StoreError> {
        mnemosyne_core::validate_name(name, "entity").map_err(|e| StoreError::CorruptRow {
            id: name.into(),
            reason: e.to_string(),
        })?;
        ...
        mnemosyne_core::validate_name(etype, "entity type").map_err(|e| {
            StoreError::CorruptRow {
                id: name.into(),
                reason: e.to_string(),
            }
