...
pub struct Triple {
    ...
    pub support: Option<mnemosyne_core::support::Support>,
...
impl Triple {
    ...
    pub fn grounding(&self) -> mnemosyne_core::support::Grounding {
        mnemosyne_core::support::Support::grounding(self.support.as_ref())
    }
}
...
pub(crate) fn kg_term_at_rest(
    ...
    if !matches!(vault.level(), mnemosyne_vault::SecurityLevel::Sealed) {
...
/// column gave them.
fn entity_id(vault: &mnemosyne_vault::Vault, secret: &[u8; 32], name: &str) -> String {
    if !matches!(vault.level(), mnemosyne_vault::SecurityLevel::Sealed) {
...
fn entity_name_at_rest(
    ...
    let sealed = matches!(vault.level(), mnemosyne_vault::SecurityLevel::Sealed)
...
fn check_authority_declaration(
    ...
    if let Some(k) = canonical_key {
        mnemosyne_core::validate_name(k, "canonical_key").map_err(|e| e.to_string())?;
    }
    Ok(())
}
...
impl PalaceStore {
    ...
    fn blind_existing_kg_rows(&self) -> Result<(), StoreError> {
        if !matches!(self.vault.level(), mnemosyne_vault::SecurityLevel::Sealed) {
            return Ok(());
        }
        ...
        if skipped > 0 {
            mnemosyne_obs::diag_warn!(
                "{skipped} content fingerprint(s) failed their own receipt HMAC and were left \
                 ...
                 retries. Run `mnemosyne verify` to see them"
            );
            return Ok(());
        }
        self.conn.execute(
    ...
    /// this is the screen on it. `mnemosyne_kg_add` is on the MCP surface
    /// and `mnemosyne_kg_query` reads objects straight back, so with
    /// `MNEMOSYNE_ADMISSION=quarantine` declared, an agent whose
    /// `mnemosyne_save` was diverted could put the same text in a fact's
    ...
    fn screen_kg_object(
        ...
        mnemosyne_core::validate_content_len(object)
            .map_err(|e| StoreError::Invalid(format!("fact {subject}/{predicate}: {e}")))?;
        if !self.admission_quarantine {
            return Ok(());
        }
        let signals = mnemosyne_core::admission::screen(object);
        if signals.is_empty() {
            return Ok(());
        }
    ...
    pub fn kg_add_grounded(
        ...
        confidence: f64,
        source: (&str, &str),
        support: Option<&mnemosyne_core::support::Support>,
        extractor: Option<&str>,
    ) -> Result<String, StoreError> {
    ...
    fn kg_add_inner(
        ...
        let _span = mnemosyne_obs::scope("kg", self.vault.id());
        let secret = self.kg_secret()?;
        ...
        mnemosyne_core::validate_name(subject, "subject")
            .map_err(|e| StoreError::Invalid(e.to_string()))?;
        mnemosyne_core::validate_name(predicate, "predicate")
            .map_err(|e| StoreError::Invalid(e.to_string()))?;
        ...
        if entity.is_some() {
            mnemosyne_obs::kg_write(mnemosyne_obs::KgKind::Entity);
        }
    ...
    pub fn kg_import(&mut self, exp: &TripleExport) -> Result<String, StoreError> {
        let secret = self.kg_secret()?;
        let t = &exp.triple;
        mnemosyne_core::validate_name(&t.subject, "subject")
            .map_err(|e| StoreError::Invalid(e.to_string()))?;
        mnemosyne_core::validate_name(&t.predicate, "predicate")
            .map_err(|e| StoreError::Invalid(e.to_string()))?;
    ...
    pub fn kg_import_entity(&mut self, name: &str, etype: &str) -> Result<(), StoreError> {
        let secret = self.kg_secret()?;
        mnemosyne_core::validate_name(name, "entity")
            .map_err(|e| StoreError::Invalid(e.to_string()))?;
        ...
        mnemosyne_core::validate_name(etype, "entity type")
            .map_err(|e| StoreError::Invalid(e.to_string()))?;
        let now = now_rfc3339();
        let tx = self.conn.transaction()?;
    ...
    pub fn kg_entities(
        ...
        let sealed = matches!(self.vault.level(), mnemosyne_vault::SecurityLevel::Sealed);
...
impl TripleRow {
    ...
    fn terms(&self, vault: &mnemosyne_vault::Vault) -> Result<(String, String), StoreError> {
        match &self.terms {
...
mod tests {
    ...
    fn grounding_survives_a_round_trip() {
        use mnemosyne_core::support::Grounding;
    ...
    fn src_drawer(content: &str) -> mnemosyne_core::Drawer {
        mnemosyne_core::Drawer::new("w", "r", content.into(), Some("t.md".into()), 0, "t")
    }
    ...
    /// `mnemosyne_save` diverted, `mnemosyne_kg_add` did not exist as far
    /// as admission was concerned, and `mnemosyne_kg_query` read the object
    /// back verbatim.
    #[test]
