...
//!
//! **The inverted tier** activates past `MNEMOSYNE_FDE_IVF_MIN` coded
...
/// (`MNEMOSYNE_FDE_IVF_MIN` set without a parseable number falls back
...
impl PalaceStore {
    ...
    fn fde_backfill(&self, model: &str) -> Result<(), StoreError> {
        ...
        if let Err(e) = filled {
            ...
            mnemosyne_obs::diag_warn!("FDE backfill rolled back ({e}); rows stay missing");
        }
        Ok(())
    }
