...
impl PalaceStore {
    ...
    pub fn set_wing_trust(&mut self, wing: &str, trust: &str) -> Result<(), StoreError> {
        ...
        mnemosyne_core::validate_trust(trust).map_err(|e| StoreError::Invalid(e.to_string()))?;
        let now = now_rfc3339();
        let tag = self
            .vault
    ...
    pub(crate) fn trust_clause(&self, floor: &str) -> Result<Option<TrustClause>, StoreError> {
        ...
        let floor_rank = mnemosyne_core::trust_rank(floor);
        if floor_rank == 0 {
            // Everything meets the lowest floor.
            return Ok(None);
        }
        if floor_rank <= mnemosyne_core::trust_rank("standard") {
            let excluded: Vec<String> = assigned
                .into_iter()
                .filter(|(_, t)| mnemosyne_core::trust_rank(t) < floor_rank)
                .map(|(w, _)| w)
                .collect();
    ...
    pub fn update_drawer(
        ...
        // (`MNEMOSYNE_ADMISSION_LLM=advisory`) is a live model call: when
    ...
    ///   into that field — so with `MNEMOSYNE_ADMIT_TRUSTED_SOURCES=cli`
    ...
    pub fn create_tunnel(
        ...
        // strings).
        mnemosyne_core::validate_name(from_wing, "from_wing")
            .map_err(|e| StoreError::Invalid(e.to_string()))?;
        mnemosyne_core::validate_name(to_wing, "to_wing")
            .map_err(|e| StoreError::Invalid(e.to_string()))?;
        ...
        // checks passed and `mnemosyne_follow_tunnel` handed an agent the
        ...
        for ((w, r), ds) in rooms {
            ...
            let span = match (dates.first(), dates.last()) {
                (Some(a), Some(b)) => {
                    ...
                    // This is reached by `mnemosyne_get_closet_index` over
