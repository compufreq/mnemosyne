...
/// `MNEMOSYNE_ORCH_RATE_LIMIT` is set to a positive integer. Single-writer
...
/// Read `MNEMOSYNE_ORCH_RATE_LIMIT`: unset, empty, `off` or `0` = off;
...
fn resolve_rate_limit(env: Option<&str>) -> anyhow::Result<u64> {
    ...
    v.parse::<u64>().map_err(|_| {
        anyhow::anyhow!(
            "MNEMOSYNE_ORCH_RATE_LIMIT={v:?} — expected requests per minute as a \
...
impl RateLimiter {
    pub(crate) fn from_env() -> anyhow::Result<Self> {
        let per_minute =
            resolve_rate_limit(std::env::var("MNEMOSYNE_ORCH_RATE_LIMIT").ok().as_deref())?;
        Ok(Self {
            per_minute,
...
pub fn serve(orch: &Orch, addr: &str, role: Role<'_>) -> anyhow::Result<()> {
    ...
    eprintln!("mnemosyne-orchestrator ({mode}) listening on http://{addr}");
...
fn data_plane(
    ...
    let creds = match orch.instance_creds(&tenant.instance) {
        ...
        // wrong `MNEMOSYNE_ORCH_KEY` or a tampered credential blob is a
...
fn delete_tenant(orch: &Orch, id: &str) -> Response<std::io::Cursor<Vec<u8>>> {
    ...
    // failure — a tampered blob, a wrong `MNEMOSYNE_ORCH_KEY`, an unknown
...
pub(crate) fn migrate_tenant(
    ...
    let manifest = ndjson
        ...
        .and_then(|v| v.get("mnemosyne_manifest").cloned());
...
mod tests {
    ...
    fn an_export_carrying_review_evidence_is_refused_not_rewritten() {
        ...
        let manifest = r#"{"mnemosyne_manifest":{"version":1,"counts":{"drawers":2}}}"#;
    ...
    fn state_failures_are_classified_by_the_error_not_the_call_site() {
        ...
        assert!(
            said.contains("MNEMOSYNE_ORCH_KEY"),
