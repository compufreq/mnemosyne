...
use mnemosyne_store::{PalaceStore, SearchHit, SearchOptions, StoreError};
...
pub fn locale_from(body: &Value) -> mnemosyne_core::temporal::Locale {
...
/// The vocabulary itself lives on [`mnemosyne_store::MorphLang::CODES`] so the
/// tool schema that advertises it cannot fall behind the parser.
pub fn morph_lang_from(body: &Value) -> mnemosyne_store::MorphLang {
    mnemosyne_store::MorphLang::declared(body.get("language").and_then(Value::as_str))
}

/// One line of prose per declarable value, for a tool schema or `--help`.
pub fn language_codes() -> String {
    mnemosyne_store::MorphLang::CODES.join(", ")
}
...
mod tests {
    ...
    fn the_evidence_line_names_every_channel_and_keeps_them_apart() {
        let hit = SearchHit {
            drawer: mnemosyne_core::Drawer::new("w", "r", "text".into(), None, 0, "test"),
            score: 0.5,
            semantic: 0.61,
            lexical: 0.77,
            lexical_exact: 0.82,
