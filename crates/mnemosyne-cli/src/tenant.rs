    ...
    fn refine(&mut self, id: &str, req: &Request, body: &str, now: i64) -> RestResult {
        ...
        for d in &sources {
            let anchor = d
                ...
                .content_date
                .as_deref()
                .and_then(mnemosyne_core::temporal::parse_anchor);
            let triples = match llm.extract_triples(&d.content) {
                Ok(t) => t,
                Err(e) => {
                    mnemosyne_obs::diag_error!("refine: triples failed for {}: {e}", d.id);
                    failed += 1;
                    continue;
                }
            };
            for t in triples {
                ...
                let dated = t.when.as_deref().and_then(|claim| {
                    mnemosyne_core::temporal::resolve_claimed_span(&d.content, claim, anchor)
                });
                if dated.is_some() {
                    dated_from_text += 1;
                }
                ...
                let support = mnemosyne_core::support::Support::evaluate(
                    &d.content,
                    t.quote
                        .as_deref()
                        .map(|q| [q])
...
mod tests {
    ...
    fn rotate_and_delete_refuse_the_vault_this_process_also_serves_over_mcp() {
        ...
        let factory: EmbedderFactory = Box::new(|_: &Vault| {
            Ok(Box::new(mnemosyne_core::HashEmbedder)
