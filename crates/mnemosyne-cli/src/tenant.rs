...
use mnemosyne_store::{PalaceStore, SearchOptions, StoreError};
...
fn elapsed_days(content_date: &Option<String>, as_of: Option<&str>) -> Option<i64> {
    ...
    mnemosyne_core::temporal::days_between(d, a)
}
...
fn elapsed_calendar(
    ...
    (
        mnemosyne_core::temporal::calendar_weeks_between(d, a),
        mnemosyne_core::temporal::calendar_months_between(d, a),
    )
}
...
fn triples_json(triples: Vec<mnemosyne_store::Triple>, want: Option<&str>) -> Value {
    let rows: Vec<Value> = triples
        .into_iter()
        .filter_map(|t| {
            let label = match t.grounding() {
                mnemosyne_core::support::Grounding::Stated => "stated",
                mnemosyne_core::support::Grounding::Background => "background",
                mnemosyne_core::support::Grounding::Unevaluated => "unevaluated",
            };
...
fn mentions_with_elapsed(
    mentions: &[mnemosyne_core::temporal::TimeMention],
    as_of: Option<&str>,
) -> Vec<Value> {
    use mnemosyne_core::temporal::{days_between, describe_interval};
    mentions
        .iter()
        .map(|m| {
...
pub type RerankerFactory = Box<dyn Fn() -> Box<dyn mnemosyne_core::rerank::Reranker + Send + Sync>>;
...
impl Tenancy {
    ...
    /// `MNEMOSYNE_ASSERTION_SECRET`, and an env var is process-global, so a
    ...
    /// `MNEMOSYNE_ASSERTION_SECRET` — precisely to stop one bearer from
    ...
    pub fn handle(&mut self, mut req: Request, now: i64) {
        ...
        let _span = mnemosyne_obs::scope_request(route_label, None);
    ...
    fn stats(&mut self, id: &str, req: &Request, now: i64) -> RestResult {
        ...
        mnemosyne_obs::set_gauge("drawers", id, full.records as f64);
        ...
        mnemosyne_obs::set_gauge("audit_chain_height", id, full.writes as f64);
    ...
    fn save_drawer(&mut self, id: &str, req: &Request, body: &str, now: i64) -> RestResult {
        ...
        if let Some(k) = kind.as_deref() {
            mnemosyne_core::validate_kind(k).map_err(|e| RestError::new(400, e.to_string()))?;
        }
    ...
    fn search(&mut self, id: &str, req: &Request, body: &str, now: i64) -> RestResult {
        ...
        // [`review_door`].
        if opts.wing.as_deref() == Some(mnemosyne_store::QUARANTINE_WING) {
            ...
            .map(|h| {
                json!({
                    ...
                    "same_frame": h
                        ...
                        .as_deref()
                        .zip(as_of.as_deref())
                        .map(|(d, a)| mnemosyne_core::temporal::same_frame(d, a)),
                    "score": h.score,
                    "semantic": h.semantic,
                    "lexical": h.lexical,
    ...
    fn delete_drawer(&mut self, id: &str, drawer_id: &str, req: &Request, now: i64) -> RestResult {
        ...
        // and MCP `mnemosyne_delete_drawer` both treat it as an error, and
    ...
    fn list_drawers(&mut self, id: &str, req: &Request, now: i64) -> RestResult {
        ...
        if wing.as_deref() == Some(mnemosyne_store::QUARANTINE_WING) {
    ...
    fn update_drawer(
        ...
        match store
            ...
            .map_err(store_err)?
        {
            mnemosyne_store::UpdateOutcome::Updated => {
                Ok((200, Body::Json(json!({ "id": drawer_id, "updated": true }))))
            }
            mnemosyne_store::UpdateOutcome::Quarantined => Ok((
                202,
                Body::Json(json!({ "id": drawer_id, "updated": false,
                                    "quarantined": true })),
            )),
            mnemosyne_store::UpdateOutcome::NotFound => Err(RestError::new(404, "no such drawer")),
        }
    }
    ...
    fn anchor(&mut self, id: &str, req: &Request, now: i64) -> RestResult {
        ...
        let behind_by = match state {
            mnemosyne_store::AnchorState::Healed { behind_by } => behind_by,
            _ => 0,
        };
        ...
        Ok((
            200,
            Body::Json(json!({
                "anchored": !matches!(state, mnemosyne_store::AnchorState::Unseeded),
                "behind_by": behind_by,
    ...
    fn rotate(&mut self, id: &str, req: &Request, now: i64) -> RestResult {
        self.assert_or_401(id, req, now)?;
        self.deny_co_resident(id, "rotating keys", "run `mnemosyne vault rotate <name>`")?;
        ...
        Ok((
            ...
            Body::Json(json!({
                ...
                "chain_head": chain_head,
                "note": "remote index copies are stale; re-run `mnemosyne index push` if used",
            })),
        ))
    }
    ...
    /// `MNEMOSYNE_ADMISSION=quarantine` on the engine.
    ...
    fn set_trust(&mut self, id: &str, req: &Request, body: &str, now: i64) -> RestResult {
        ...
        mnemosyne_core::validate_trust(&trust).map_err(|e| RestError::new(400, e.to_string()))?;
        let store = self.store_for(id)?;
    ...
    ///
    /// Requires `MNEMOSYNE_LLM_URL` — without it the vault is untouched and
    ...
    fn refine(&mut self, id: &str, req: &Request, body: &str, now: i64) -> RestResult {
        ...
        // into the knowledge graph, where `mnemosyne_kg_query` hands it
        ...
        if wing == Some(mnemosyne_store::QUARANTINE_WING) {
            return Err(RestError::new(
                400,
                format!(
                    ...
                    mnemosyne_store::QUARANTINE_WING
                ),
            ));
        }

        let llm =
            mnemosyne_llm::LlmClient::from_env().map_err(|e| RestError::new(400, e.to_string()))?;
        let store = self.store_for(id)?;

        // The distillation itself lives in `crate::refine`, driven
        // identically by `mnemosyne refine` — same LLM configuration must
    ...
    fn export(&mut self, id: &str, req: &Request, now: i64) -> RestResult {
        ...
        let mut counts = mnemosyne_vault::bundle::ManifestCounts::default();
        ...
        let manifest = mnemosyne_vault::bundle::BundleManifest {
            version: 1,
            vault: vault_id,
            level,
            ...
            expires: None,
            sender: None,
            payload_sha256: mnemosyne_vault::bundle::payload_digest(out.as_bytes()),
            sig: None,
        };
        let framed = mnemosyne_vault::bundle::frame_payload(&manifest, out.as_bytes());
        ...
        if self.read_only {
            mnemosyne_obs::diag_warn!("export served read-only; egress not chain-audited");
        } else {
    ...
    fn import(&mut self, id: &str, req: &Request, body: &str, now: i64) -> RestResult {
        ...
        let (manifest, record_bytes) = mnemosyne_vault::bundle::split_payload(body.as_bytes())
        ...
        let mut kg_records: Vec<mnemosyne_store::TripleExport> = Vec::new();
        ...
        for (n, line) in body.lines().enumerate() {
            ...
            let tok = match obj.get("tok") {
                Some(t) => {
                    ...
                    if mnemosyne_core::late::dequantize_tokens(&packed).is_none() {
                        return Err(RestError::new(
                            400,
        ...
        for (n, (drawer, vector, tok)) in records.iter().enumerate() {
            ...
            let out = store
                .import_record(drawer, vector.clone(), mnemosyne_store::IMPORT_SURFACE)
    ...
    fn store_for(&mut self, vault_id: &str) -> Result<&mut PalaceStore, RestError> {
        if !self.stores.contains_key(vault_id) {
            ...
            let vault = if self.read_only {
                self.manager
                    .unlock_as(vault_id, mnemosyne_vault::Access::ReadOnly)
            } else {
            ...
            // MNEMOSYNE_RETRIEVAL=fde silently served the default instead —
            ...
            match std::env::var("MNEMOSYNE_RETRIEVAL").as_deref() {
                ...
                Ok("hnsw") => {
                    return Err(RestError::new(
                        500,
                        "MNEMOSYNE_RETRIEVAL=hnsw is not available on the multi-tenant                          server (in-process index); use pq or fde, or serve a single vault",
                    ))
                }
                Ok("") | Err(_) => {}
                Ok(other) => {
                    return Err(RestError::new(
                        500,
                        format!("unknown MNEMOSYNE_RETRIEVAL {other:?} (expected: pq, fde)"),
                    ))
                }
            }
...
/// is readable only from the operator seat (`mnemosyne drawer get <id>` on
...
///   a MULTI-tenant engine started without `MNEMOSYNE_ASSERTION_SECRET`
...
fn review_door(under_assertions: bool, named_wing: Option<&str>) -> Result<(), RestError> {
    ...
    if named_wing != Some(mnemosyne_store::QUARANTINE_WING) {
        return Err(RestError::new(
            403,
            format!(
        ...
        return Err(RestError::new(
            ...
            format!(
                "the {} queue is not readable under per-vault assertions: an assertion \
                 ...
                 operator seat with `mnemosyne drawer get <drawer-id>`",
                mnemosyne_store::QUARANTINE_WING
            ),
        ));
    }
...
/// `mnemosyne_vault::bundle::BundleManifest::attest` (ROADMAP C5).
///
...
/// that route verified unconditionally while `mnemosyne import` verified
...
fn verify_attestation(
    manifest: Option<&mnemosyne_vault::bundle::BundleManifest>,
    pinned: Option<&str>,
) -> Result<mnemosyne_vault::bundle::Attestation, RestError> {
    mnemosyne_vault::bundle::BundleManifest::attest(manifest, pinned)
...
/// for those two.
fn vault_err(e: mnemosyne_vault::VaultError) -> RestError {
...
fn store_err(e: StoreError) -> RestError {
    let code = match &e {
        ...
        StoreError::Vault(
            ...
            | mnemosyne_vault::VaultError::CorruptManifest(_),
        ) => 409,
...
mod tests {
    ...
    fn embedder_factory() -> EmbedderFactory {
        Box::new(|_v: &Vault| {
            Ok(Box::new(mnemosyne_core::embed::HashEmbedder)
                as Box<dyn mnemosyne_core::embed::Embedder + Send>)
        })
    }
    ...
    /// `mnemosyne_kg_query` hands to any agent. The read fence would be
    ...
    fn the_fence_reads_the_wing_not_any_string_that_looks_like_it() {
        ...
        // assertions.
        assert!(review_door(false, Some(mnemosyne_store::QUARANTINE_WING)).is_ok());
        assert!(review_door(true, Some(mnemosyne_store::QUARANTINE_WING)).is_err());
    ...
    /// than declared through the environment, because `MNEMOSYNE_ADMISSION`
    ...
    fn a_diverted_save_answers_202_with_the_landed_id_on_every_v1_arm() {
        ...
        // --- the external-vault arm ---------------------------------------
        {
            ...
            let mut store = PalaceStore::open_with_embedder(
                vault,
                Box::new(mnemosyne_core::ExternalEmbedder::new("acme-embed", 8)),
            )
            .unwrap();
            store.set_admission(true);
    ...
    fn an_imported_token_artifact_follows_the_row_and_refuses_a_forged_id() {
        ...
        let dim = 8usize;
        let packed = mnemosyne_core::late::quantize_tokens(&vec![0.5f32; dim * 4], dim);
    ...
    fn a_wrapped_manifest_verdict_is_409_not_500() {
        ...
        let io = StoreError::Vault(mnemosyne_vault::VaultError::Io(std::io::Error::other("x")));
