...
/// still appears in `mnemosyne_list_wings`/`mnemosyne_get_taxonomy` with
...
const CLOSES_A_VALIDITY_WINDOW: &[&str] = &["mnemosyne_kg_invalidate", "mnemosyne_kg_supersede"];
...
/// taking `mnemosyne_kg_set_authority` off the surface satisfied only its
    ...
    for t in store.kg_query_entity(subject, None, "outgoing")? {
        ...
        anyhow::bail!(
            "{tool}: ({subject}, {predicate}) is held by approved canonical fact \
             ...
             exact-authority door `mnemosyne_lookup_canonical` reads, so the \
             authority tier is an operator surface in both directions: \
             `mnemosyne kg authority {} --class stated --review rejected` (or \
...
fn tool_definitions() -> Value {
    ...
    json!([
        ...
        tool("mnemosyne_list_tunnels", "List tunnels, optionally touching one wing.",
            json!({ "wing": s("filter") }), &[]),
        tool("mnemosyne_history", "Audit-chain history for a memory or fact: what happened to it, when, and the tamper tag as of each write. Pass `subject` (a drawer id, fact id or entity id) to trace one record, or omit it for recent activity. Never returns content. Operator-only namespaces (review rulings, trust and retention policy, destructions, exports, read audits, rotations) are not visible here.",
    ...
    match name {
        ...
        "mnemosyne_update_drawer" => {
            ...
            match store.update_drawer(id, req_str(args, "content")?, "mcp")? {
                mnemosyne_store::UpdateOutcome::Updated => Ok(format!("updated drawer {id}")),
                mnemosyne_store::UpdateOutcome::Quarantined => Ok(format!(
        ...
        "mnemosyne_history" => {
            let rows = store.history(
                mnemosyne_store::manage::HistoryScope::Agent,
        ...
        "mnemosyne_diary_write" => {
            ...
            // a diverted entry is not readable by `mnemosyne_diary_read`,
...
mod tests {
    ...
    fn mcp_cannot_read_rule_on_or_destroy_the_review_queue() {
        ...
        for (tool, args) in [
            ("mnemosyne_get_drawer", json!({ "id": clean.id })),
            ("mnemosyne_list_drawers", json!({ "wing": "notes" })),
            (
                "mnemosyne_search",
        ...
        for tool in [
            ...
            let (err, text) = call(
                &mut h,
                tool,
                json!({ "query": "OK", "wing": mnemosyne_store::QUARANTINE_WING }),
            );
    ...
    fn search(store: &mut PalaceStore, args: Value) -> String {
        call_direct(store, "mnemosyne_search", args)
    }
    ...
    fn the_search_tool_honours_every_declaration_its_schema_advertises() {
        ...
        for code in mnemosyne_store::MorphLang::CODES {
            assert!(
    ...
    ///
    /// Taking `mnemosyne_kg_set_authority` off the surface closed the forgery
    ...
    fn a_search_names_its_exclusions_and_its_hits_ids() {
        ...
        assert!(
            ...
            "the hit must carry the id `mnemosyne_get_drawer` takes:\n{out}"
        );
