...
//!
//! `mnemosyne refine` and `POST /v1/vaults/{id}/refine` are the same
//! capability under the same `MNEMOSYNE_LLM_*` configuration, and they
...
use mnemosyne_llm::LlmClient;
use mnemosyne_store::{PalaceStore, StoreError, QUARANTINE_WING};
...
pub(crate) fn refine(
    ...
    // writes it into the knowledge graph, where `mnemosyne_kg_query` hands it
    ...
    if opts.wing == Some(QUARANTINE_WING) {
        return Err(StoreError::Invalid(format!(
            "refine cannot be scoped to {QUARANTINE_WING}: its residents are \
             ...
             (`mnemosyne admission allow|deny`, or `POST \
    ...
    for d in &sources {
        ...
        for t in triples {
            ...
            if mnemosyne_core::validate_name(&subject, "subject").is_err()
                || mnemosyne_core::validate_name(&predicate, "predicate").is_err()
            {
                rep.skipped += 1;
                continue;
            }
...
mod tests {
    ...
    /// `mnemosyne refine --wing quarantine-pending` was a door out of the
    ...
    /// extractor and written into the graph, where `mnemosyne_kg_query`
