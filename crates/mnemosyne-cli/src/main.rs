...
use i18n::{fill, tr};
use mnemosyne_core::normalize::mode_for_path;
use mnemosyne_core::{chunk_text, normalize_content, ChunkOptions, Drawer};
...
enum Command {
    ...
    Remember {
        ...
        /// update link (check with `mnemosyne verify`); the old drawer is
    ...
    /// surface; enable screening with MNEMOSYNE_ADMISSION=quarantine)
    Admission {
        #[command(subcommand)]
    ...
    ServeHttp {
        ...
        /// runs and MNEMOSYNE_READ_AUDIT records nothing
        #[arg(long)]
        read_only: bool,
    },
    ...
    Repair {
        ...
        /// MNEMOSYNE_RERANKER=colbert or colbert-ort): encodes every drawer
        /// missing one,
...
fn build_export_payload(
    store: &mnemosyne_store::PalaceStore,
    signing_secret: Option<&str>,
    trust: Option<&str>,
    expires: Option<&str>,
) -> Result<Vec<u8>> {
    use mnemosyne_vault::bundle;
...
fn upsert_batched(
    ...
    drawers: &[Drawer],
) -> Result<mnemosyne_store::BulkOutcome> {
    let mut total = mnemosyne_store::BulkOutcome::default();
...
/// batch. `mnemosyne import` printed "imported 500" while an arbitrary
    ...
    if quarantined > 0 {
        println!(
            "{quarantined} of these tripped the admission screen and were quarantined \
             ...
             Review with `mnemosyne admission list`."
        );
    }
}
...
/// dropping the PQ/IVF tables) and, under `MNEMOSYNE_READ_AUDIT=chain`,
...
fn open_store_as(cli: &Cli, vault: &str, posture: Posture) -> Result<PalaceStore> {
    ...
    let v = match posture {
        Posture::ReadOnly => mgr.unlock_as(vault, mnemosyne_vault::Access::ReadOnly)?,
    ...
    let open = |v: Vault, e: Box<dyn mnemosyne_core::embed::Embedder + Send>| match posture {
    ...
    let mut store = match std::env::var("MNEMOSYNE_EMBEDDER").as_deref() {
        ...
        Ok("ort") => {
            #[cfg(feature = "ort")]
            {
                let embedder = mnemosyne_embed_ort::embedder_from_env()
                    .map_err(|e| anyhow::anyhow!("loading ORT embedder: {e}"))?;
                open(v, Box::new(embedder))?
            }
            #[cfg(not(feature = "ort"))]
            bail!(
                "MNEMOSYNE_EMBEDDER=ort requires a build with the 'ort' feature \
        ...
        Ok("http") => {
            let embedder = mnemosyne_llm::HttpEmbedder::from_env()
                .map_err(|e| anyhow::anyhow!("connecting to the embeddings endpoint: {e}"))?;
            open(v, Box::new(embedder))?
        }
        Ok("hash") | Ok("") | Err(_) => open(v, Box::new(mnemosyne_core::HashEmbedder))?,
        Ok(other) => {
...
/// (`MNEMOSYNE_ADMISSION_LLM=advisory` + the `MNEMOSYNE_LLM_*` family).
...
pub(crate) fn attach_admission_advisor(store: &mut PalaceStore) -> Result<()> {
    if let Some(advisor) = mnemosyne_llm::advisor::LlmAdmissionAdvisor::from_env()
...
fn attach_retrieval(store: &mut PalaceStore) -> Result<()> {
    match std::env::var("MNEMOSYNE_RETRIEVAL").as_deref() {
        ...
        Ok("") | Err(_) => {}
        Ok(other) => bail!("unknown MNEMOSYNE_RETRIEVAL {other:?} (expected: pq, fde, hnsw)"),
    }
    Ok(())
}

/// Attach the second retrieval stage via `MNEMOSYNE_RERANKER`: `onnx` /
/// `colbert` load the tract backend (`onnx` feature), `ort` / `colbert-ort`
/// the ONNX Runtime backend (`ort` feature) — same model files and
/// `MNEMOSYNE_RERANK_*` / `MNEMOSYNE_COLBERT_*` variables either way.
...
fn attach_reranker(store: &mut PalaceStore) -> Result<()> {
    match std::env::var("MNEMOSYNE_RERANKER").as_deref() {
        ...
        Ok("ort") => {
            #[cfg(feature = "ort")]
            {
                let rr = mnemosyne_embed_ort::reranker_from_env()
            ...
            #[cfg(not(feature = "ort"))]
            bail!(
                "MNEMOSYNE_RERANKER=ort requires a build with the 'ort' feature \
                 (cargo build -p mnemosyne-cli --features ort)"
            );
        }
        Ok("colbert-ort") => {
            #[cfg(feature = "ort")]
            {
                let c = mnemosyne_embed_ort::colbert_from_env()
            ...
            #[cfg(not(feature = "ort"))]
            bail!(
                "MNEMOSYNE_RERANKER=colbert-ort requires a build with the 'ort' feature \
                 (cargo build -p mnemosyne-cli --features ort)"
            );
        }
        Ok("") | Err(_) => Ok(()),
        Ok(other) => {
            bail!(
                "unknown MNEMOSYNE_RERANKER {other:?} \
            ...
            match std::env::var("MNEMOSYNE_EMBEDDER").as_deref() {
                ...
                Ok("ort") => {
                    #[cfg(feature = "ort")]
                    {
                        ...
                        static SHARED: OnceLock<Arc<mnemosyne_embed_ort::OrtEmbedder>> =
                            OnceLock::new();
                        let arc = match SHARED.get() {
                            Some(a) => a.clone(),
                            None => {
                                let e = mnemosyne_embed_ort::embedder_from_env()
                                    .map_err(|e| anyhow::anyhow!("loading ORT embedder: {e}"))?;
...
/// A cheap handle onto the one shared [`mnemosyne_embed_ort::OrtEmbedder`]
/// session pool the multi-tenant server loaded.
#[cfg(feature = "ort")]
struct SharedOrtEmbedder(std::sync::Arc<mnemosyne_embed_ort::OrtEmbedder>);

#[cfg(feature = "ort")]
impl mnemosyne_core::embed::Embedder for SharedOrtEmbedder {
    ...
    match std::env::var("MNEMOSYNE_RERANKER").as_deref() {
        ...
        Ok("ort") => {
            #[cfg(feature = "ort")]
            {
                use std::sync::Arc;
                let shared = Arc::new(
                    mnemosyne_embed_ort::reranker_from_env()
                        .map_err(|e| anyhow::anyhow!("loading ORT reranker: {e}"))?,
                );
                let factory: tenant::RerankerFactory =
        ...
        Ok("colbert") | Ok("colbert-ort") => bail!(
            "the ColBERT late-interaction stage is not available on the \
             multi-tenant server (use MNEMOSYNE_RERANKER=onnx or ort, or serve \
             a single vault)"
        ),
        Ok("") | Err(_) => Ok(None),
        Ok(other) => bail!("unknown MNEMOSYNE_RERANKER {other:?} (expected: onnx, ort, or unset)"),
    }
}

/// A cheap handle onto the one shared [`mnemosyne_embed_ort::OrtReranker`] the
...
#[cfg(feature = "ort")]
struct SharedOrtReranker(std::sync::Arc<mnemosyne_embed_ort::OrtReranker>);

#[cfg(feature = "ort")]
impl mnemosyne_core::rerank::Reranker for SharedOrtReranker {
    fn model_name(&self) -> &str {
        self.0.model_name()
    }
...
/// checking begins: `mnemosyne search`, `mnemosyne recent`, `mnemosyne stats`
...
///
/// **The stated cost**: a wrong `MNEMOSYNE_PASSPHRASE` derives a different
...
fn integrity_verdict(e: &anyhow::Error) -> bool {
    use mnemosyne_store::StoreError as S;
    use mnemosyne_vault::VaultError as V;
    ...
    match &cli.command {
        ...
        Command::Bundle { action } => match action {
            BundleAction::Keygen { out } => {
                ...
                let (secret_hex, recipient_hex) = mnemosyne_vault::bundle::keygen();
                ...
                println!("Seal an export with: mnemosyne export --to {recipient_hex} --out palace.bundle");
            }
            BundleAction::Recipient { identity } => {
                ...
                let recipient = mnemosyne_vault::bundle::recipient_of(&secret)
            ...
            BundleAction::SignKeygen { out } => {
                ...
                let (secret_hex, sender_hex) = mnemosyne_vault::bundle::sign_keygen();
                write_identity(&path, &secret_hex)?;
                println!(
                ...
                println!(
                    "Sign an export with: mnemosyne export --sign {} …; \
                     verify with: mnemosyne import --sender {sender_hex} …",
                    path.display()
                );
            }
            BundleAction::Sender { identity } => {
                ...
                let sender = mnemosyne_vault::bundle::signer_of(&secret)
                    .map_err(|e| anyhow::anyhow!("{e}"))?;
                println!("{sender}");
            }
        },
        Command::Vault { action } => match action {
            ...
            VaultAction::Anchor { name } => {
                use mnemosyne_store::AnchorState;
            ...
            VaultAction::Rotate { name } => {
                ...
                println!(
                    "If this vault was pushed to a remote index, re-run: mnemosyne index push"
                );
            }
        },
        Command::Remember {
        ...
        Command::Search {
            ...
            let opts = SearchOptions {
                ...
                morph_lang: mnemosyne_store::MorphLang::declared(language.as_deref()),
                wing: wing.clone(),
                room: room.clone(),
                kind: kind.clone(),
        ...
        Command::Verify { vault } => {
            ...
            if !links.is_empty() {
                use mnemosyne_store::ReceiptVerdict as V;
                ...
                Some(path) => {
                    ...
                    println!(
                        "{} drawer(s) destroyed; attestation written to {} \
                         (verify with: mnemosyne verify-forgetting {})",
                        att.drawers.len(),
                        path.display(),
                        path.display()
                    );
        ...
        Command::VerifyForgetting { file, vault } => {
            ...
            let att: mnemosyne_store::ForgetAttestation = serde_json::from_str(&raw)
            ...
            if let Err(e) = store.verify_forget_attestation(&att) {
                match e {
                    mnemosyne_store::StoreError::Attestation(why) => {
                        ...
                        Some(path) => {
                            ...
                            println!(
                                "Denied: content destroyed, ruling audited; attestation \
                                 written to {} (verify with: mnemosyne verify-forgetting {})",
                                path.display(),
                                path.display()
                            );
                        }
        ...
        Command::History {
            ...
            let rows = store.history(
                mnemosyne_store::manage::HistoryScope::Operator,
                subject.as_deref(),
                *limit,
                *offset,
            )?;
        ...
        Command::Export {
            ...
            if let (Some(m), _) = mnemosyne_vault::bundle::split_payload(&payload)
                .map_err(|e| anyhow::anyhow!("{e}"))?
            {
            ...
            match to {
                Some(recipient) => {
                    ...
                    let sealed = mnemosyne_vault::bundle::encrypt_for(recipient, &payload)
        ...
        Command::Import {
            ...
            let text = if mnemosyne_vault::bundle::is_bundle(&raw) {
                ...
                let plain = mnemosyne_vault::bundle::decrypt_with(&secret, &raw)
            ...
            let (manifest, record_bytes) = mnemosyne_vault::bundle::split_payload(text.as_bytes())
            ...
            let attested = mnemosyne_vault::bundle::BundleManifest::attest(
                manifest.as_ref(),
                sender.as_deref(),
            )
            ...
            let mut kg_batch: Vec<mnemosyne_store::TripleExport> = Vec::new();
            ...
            for (lineno, line) in text.lines().enumerate() {
                ...
                let drawer = if v.get("meta").is_some() {
                    // Native mnemosyne export: full Drawer JSON — including
                    ...
                    mnemosyne_store::PalaceStore::import_stamp(&d, mnemosyne_store::IMPORT_SURFACE)
                    ...
                    Drawer::new(
                        ...
                        meta.get("chunk_index")
                            ...
                            .unwrap_or(0) as u32,
                        mnemosyne_store::IMPORT_SURFACE,
                    )
        ...
        Command::Kg { action, vault } => {
            ...
            match action {
                ...
                KgAction::Receipts { problems_only } => {
                    use mnemosyne_store::ReceiptVerdict;
            ...
            match action {
                ...
                DrawerAction::Update { id, content } => {
                    match store.update_drawer(id, content, "cli")? {
                        mnemosyne_store::UpdateOutcome::Updated => println!("Updated drawer {id}"),
                        mnemosyne_store::UpdateOutcome::Quarantined => println!(
                            "Update to {id} quarantined pending review — the drawer \
                             keeps its previous content (see `mnemosyne admission list`)."
                        ),
                        mnemosyne_store::UpdateOutcome::NotFound => {
        ...
        Command::Refine {
            ...
            mnemosyne_core::validate_name(fact_room, "fact_room")?;
            let mut store = open_store(&cli, vault)?;
            // One implementation, shared with `POST /v1/vaults/{id}/refine`
            // (crate::refine): the same MNEMOSYNE_LLM_* configuration used to
            ...
            // MNEMOSYNE_LLM_URL is told that before being told the wing is
                    ...
                    if !store.verify()?.ok() {
                        println!(
                            "refusing to back up vault '{vault}': integrity verification \
                             failed (run `mnemosyne verify --vault {vault}` for the detail)"
                        );
                        std::process::exit(EXIT_INTEGRITY.into());
                    }
                    drop(store);
            ...
            match action {
                IndexAction::Push {
                    ...
                    let plaintext = if *allow_plaintext {
                        mnemosyne_store::PlaintextPush::Allow
                    } else {
                        mnemosyne_store::PlaintextPush::Refuse
                    };
                    ...
                    let kind = if store.vault().level() == mnemosyne_vault::SecurityLevel::Sealed {
                        "sealed"
                    } else {
                        "PLAINTEXT"
                    };
...
fn mine_files(
    ...
    for file in &files {
        ...
        let normalized =
            mnemosyne_core::normalize::normalize_content_mode(&text, mode_for_path(file));
...
fn mine_convos(
    ...
    for file in &files {
        ...
        for (idx, (chunk, started_at)) in
            mnemosyne_core::convo::chunk_exchanges_dated(&messages, 800)
                .into_iter()
                .enumerate()
        {
            batch.push(
...
fn sweep_path(
    ...
    for file in &files {
        ...
        for msg in mnemosyne_core::convo::parse_transcript(&text) {
            ...
            let content = format!("{}: {}", mnemosyne_core::convo::label(&msg), msg.text);
            let normalized = normalize_content(&content);
...
fn snippet(content: &str, query: &str, max: usize) -> String {
    ...
    match hit {
        ...
        Some(pos) => {
            ...
            // boundary" on an ordinary `mnemosyne search` over ordinary stored
...
mod tests {
    ...
    fn an_integrity_verdict_exits_2_where_an_ordinary_failure_stays_1() {
        ...
        let argv = |args: &[&str]| {
            let mut v = vec!["mnemosyne", "--data-dir", root.as_str()];
    ...
    /// an ordinary `mnemosyne search`, over stored text nobody had to craft.
