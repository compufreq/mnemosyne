...
//!
//! When `MNEMOSYNE_ASSERTION_SECRET` is declared, **both** transports
...
pub fn serve_http(
    ...
    loop {
        ...
        if request.method() == &Method::Get && path == "/monitor" {
            ...
            mnemosyne_obs::http_request("monitor", 200, start.elapsed());
            continue;
        }
        ...
        if request.method() == &Method::Get && path == "/ui" {
            ...
            mnemosyne_obs::http_request("ui", 200, start.elapsed());
            continue;
        }
        ...
        match (request.method().clone(), path.as_str()) {
            (Method::Post, "/mcp") => {
                ...
                // so declaring MNEMOSYNE_ASSERTION_SECRET left the
                ...
                if let Err(code) = tenancy.assert_transport(handler.vault_id(), &request, now) {
                    ...
                    mnemosyne_obs::http_request("mcp", code, start.elapsed());
                    continue;
                }
