...
/// `wing_trusts` and `retention_policies` and `mnemosyne vault rotate`
...
mod tests {
    ...
    fn operator_only_capabilities_never_reach_mcp() {
        ...
        for cap in OPERATOR_ONLY {
            ...
            // `tool("mnemosyne_{cap}` could not express "no MCP tool may
            // write the authority tier", because the tool was called
            // `mnemosyne_kg_set_authority` — so the boundary was
            ...
            let tool = format!("tool(\"mnemosyne_{cap}");
            assert!(
                !src.contains(&tool),
                "mnemosyne_{cap}* is exposed over MCP. That is an operator \
    ...
    fn every_mutating_tool_is_refused_when_read_only() {
        ...
        // `mnemosyne_kg_set_authority` from the surface left its entry behind,
        ...
        for line in write_list.lines() {
            ...
            if !name.starts_with("mnemosyne_") {
                continue;
            }
            assert!(
