...
fn mine_and_export_roundtrip() {
    ...
    let m = &manifest["mnemosyne_manifest"];
...
fn cli_import_refuses_a_broken_signature_even_with_no_sender_pinned() {
    ...
    use mnemosyne_vault::bundle::{payload_digest, BundleManifest, ManifestCounts};
    let (secret, _sender) = mnemosyne_vault::bundle::sign_keygen();
    ...
    let frame = |m: &BundleManifest| mnemosyne_vault::bundle::frame_payload(m, records.as_bytes());
...
fn wrong_passphrase_cannot_read_sealed_vault() {
    ...
    let mut c = Command::cargo_bin("mnemosyne").unwrap();
    c.env("MNEMOSYNE_HOME", home.path())
        .env("MNEMOSYNE_PASSPHRASE", "correct horse");
    ...
    let mut c = Command::cargo_bin("mnemosyne").unwrap();
    c.env("MNEMOSYNE_HOME", home.path())
        .env("MNEMOSYNE_PASSPHRASE", "wrong staple");
