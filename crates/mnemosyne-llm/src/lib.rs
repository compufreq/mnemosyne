...
pub struct LlmClient {
    ...
    /// `MNEMOSYNE_LLM_URL` at a gateway that demands one.
    key: String,
    agent: ureq::Agent,
}
...
impl LlmClient {
    ...
    /// construction with no override. `MNEMOSYNE_LLM_CA` declares a
    ...
    pub fn with_key(
        ...
        if !tls && !loopback {
            return Err(LlmError::Refused(format!(
                "cleartext http to non-loopback {base} — drawer content would \
                 ...
                 self-signed root with MNEMOSYNE_LLM_CA=<pem>."
            )));
        }
        if !loopback {
        ...
        match std::env::var("MNEMOSYNE_LLM_CA") {
            Ok(path) if !path.trim().is_empty() => {
                if tls {
                    let pem = std::fs::read(&path).map_err(|e| {
                        LlmError::Refused(format!("MNEMOSYNE_LLM_CA {path:?} cannot be read: {e}"))
                    })?;
                    builder =
                        builder.tls_config(crate::embed::pinned_roots_from_pem(&pem).map_err(
                            |e| LlmError::Refused(format!("MNEMOSYNE_LLM_CA {path:?}: {e}")),
                        )?);
                } else {
                    mnemosyne_obs::diag_warn!(
                        "MNEMOSYNE_LLM_CA is set but {base} is not https — the \
    ...
    ///
    /// `MNEMOSYNE_LLM_KEY` is optional and unset by default: local runtimes
    ...
    pub fn from_env() -> Result<Self, LlmError> {
        ...
        let model = std::env::var("MNEMOSYNE_LLM_MODEL").unwrap_or_else(|_| "llama3.2".to_string());
        ...
        let key = std::env::var("MNEMOSYNE_LLM_KEY").unwrap_or_default();
...
pub struct ExtractedTriple {
    ...
    /// `mnemosyne_core::temporal::resolve_claimed_span`, which rejects
    ...
    ///
    /// Checked by `mnemosyne_core::support::Support::evaluate`, which keeps
...
mod tests {
    ...
    fn llm_client_refuses_cleartext_to_a_non_loopback_host() {
        ...
        for needle in ["cleartext", "TLS", "MNEMOSYNE_LLM_CA", "no override"] {
