...
//! `mnemosyne_core::admission::AdmissionAdvisor` for the contract this
...
//! `MNEMOSYNE_LLM_CA` pins a self-signed root exactly as
//! `MNEMOSYNE_EMBED_CA` does for the embedder.

use mnemosyne_core::admission::AdmissionAdvisor;
...
impl LlmAdmissionAdvisor {
    /// Build when the deployment declared `MNEMOSYNE_ADMISSION_LLM=advisory`
    /// (71st env var) — the model itself comes from the existing
    /// `MNEMOSYNE_LLM_*` family. `Ok(None)` when not declared; an error
    ...
    pub fn from_env() -> Result<Option<Self>, LlmError> {
        match std::env::var("MNEMOSYNE_ADMISSION_LLM") {
            ...
            Ok(v) => {
                return Err(LlmError::Refused(format!(
                    "MNEMOSYNE_ADMISSION_LLM={v:?} — the only mode is 'advisory' \
        ...
        // Transport policy (TLS or loopback, MNEMOSYNE_LLM_CA pin) is the
...
mod tests {
    ...
    fn advisory_refuses_cleartext_non_loopback_and_unknown_modes() {
        ...
        // guaranteed-clean env (no MNEMOSYNE_ADMISSION_LLM set in CI).
