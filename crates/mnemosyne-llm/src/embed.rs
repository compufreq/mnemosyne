...
//! [`HashEmbedder`](mnemosyne_core::HashEmbedder), an ONNX file loaded from
//! disk (`onnx`/`ort`), or not at all —
//! [`ExternalEmbedder`](mnemosyne_core::ExternalEmbedder) is an *identity* for
...
//! posture. Nothing is contacted unless `MNEMOSYNE_EMBED_URL` is set.
//!
...
//! root with `MNEMOSYNE_EMBED_CA=<pem>`. Loopback HTTP stays allowed — the
//! wire never leaves the machine, local runtimes serve plain HTTP by
//! default, and the in-process test harness rides it.
//!
//! **A declared CA is a pin, not an addition.** When `MNEMOSYNE_EMBED_CA`
...
//! (`MNEMOSYNE_FORCE_EMBEDDER=1` + `repair`).
...
impl HttpEmbedder {
    /// Build from `MNEMOSYNE_EMBED_URL`, `MNEMOSYNE_EMBED_MODEL`, optional
    /// `MNEMOSYNE_EMBED_API` (`openai` | `ollama`; guessed from the URL like
    /// the LLM client does), optional `MNEMOSYNE_EMBED_KEY`, and optional
    /// `MNEMOSYNE_EMBED_DIM`.
    ///
    ...
    pub fn from_env() -> Result<Self, LlmError> {
        let base = std::env::var("MNEMOSYNE_EMBED_URL").map_err(|_| LlmError::NotConfigured)?;
        let model = std::env::var("MNEMOSYNE_EMBED_MODEL")
            .unwrap_or_else(|_| "nomic-embed-text".to_string());
        let kind = match std::env::var("MNEMOSYNE_EMBED_API").ok().as_deref() {
            Some("openai") => ApiKind::OpenAi,
            Some("ollama") => ApiKind::Ollama,
            _ if base.contains("/v1") => ApiKind::OpenAi,
            _ => ApiKind::Ollama,
        };
        let key = std::env::var("MNEMOSYNE_EMBED_KEY").unwrap_or_default();
        let declared = std::env::var("MNEMOSYNE_EMBED_DIM")
            .ok()
    ...
    pub fn connect(
        ...
        if !tls && !loopback {
            return Err(LlmError::Refused(format!(
                "cleartext http to non-loopback {base} — drawer text would cross \
                 ...
                 self-signed root with MNEMOSYNE_EMBED_CA=<pem>."
            )));
        }
        if loopback {
            mnemosyne_obs::diag_info!("embedder: {model} via {base} (loopback)");
        } else {
        ...
        match std::env::var("MNEMOSYNE_EMBED_CA") {
            Ok(path) if !path.trim().is_empty() => {
                if tls {
                    let pem = std::fs::read(&path).map_err(|e| {
                        LlmError::Refused(format!(
                            "MNEMOSYNE_EMBED_CA {path:?} cannot be read: {e}"
                        ))
                    })?;
                    builder = builder.tls_config(pinned_roots_from_pem(&pem).map_err(|e| {
                        LlmError::Refused(format!("MNEMOSYNE_EMBED_CA {path:?}: {e}"))
                    })?);
                } else {
                    ...
                    // security setting.
                    mnemosyne_obs::diag_warn!(
                        "MNEMOSYNE_EMBED_CA is set but {base} is not https — \
...
// `mnemosyne-net` since 2026-08-05 and is re-exported here so this module's
...
pub(crate) use mnemosyne_net::is_loopback;
pub(crate) use mnemosyne_net::pinned_roots as pinned_roots_from_pem;
...
mod tests {
    ...
    fn cleartext_to_a_non_loopback_host_is_refused_with_the_fix_named() {
        ...
        for needle in ["cleartext", "TLS", "MNEMOSYNE_EMBED_CA", "no override"] {
    ...
    fn a_declared_ca_pins_or_refuses_never_falls_back() {
        // A real self-signed certificate (openssl, CN=mnemosyne-test-ca).
