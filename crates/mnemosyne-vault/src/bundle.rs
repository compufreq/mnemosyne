...
//!   "mnemosyne.v2/bundle")` — an attacker must break BOTH the curve and
...
//!   accepted PQ bar).
//!   Layout: `MNEMOSYNE-BUNDLE-2` ‖ eph_pub (32) ‖ kem_ct (1088) ‖
...
//!   recipient), info = "mnemosyne.v1/bundle")`, layout
//!   `MNEMOSYNE-BUNDLE-1` ‖ eph_pub (32) ‖ nonce (24) ‖ ciphertext.
...
pub const BUNDLE_MAGIC_V2: &[u8; 18] = b"MNEMOSYNE-BUNDLE-2";
...
const MANIFEST_LINE_KEY: &str = "mnemosyne_manifest";
...
fn hybrid_file_key(
    ...
    let mut key = [0u8; KEY_LEN];
    hk.expand(b"mnemosyne.v2/bundle", &mut key)
        .expect("32 bytes is a valid HKDF-SHA256 output length");
    key
}
...
mod tests {
    ...
    fn manifest(records: &[u8]) -> BundleManifest {
        BundleManifest {
            ...
            embedder: Some("mnemosyne-hash-v3".into()),
            chain_head: None,
