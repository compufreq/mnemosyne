...
enum Command {
    ...
    Scopescale {
        ...
        #[arg(long, default_value = "131072,262144,524288,1048576")]
        sizes: String,
        /// Fixed probe-wing size (default above MNEMOSYNE_WING_PQ_MIN so
    ...
    /// model (`MNEMOSYNE_EMBEDDER=http`) is the capability under test.
    ...
    /// ingest"). Runs `mnemosyne_core::admission::screen` over every
    ...
    /// `mnemosyne` runs the native store; `mem0` / `supermemory` drive a
    ...
    Vs {
        /// Path to locomo10.json
        dataset: std::path::PathBuf,
        /// System under test: mnemosyne | mem0 | supermemory
        #[arg(long)]
        system: String,
        #[arg(short = 'k', long, default_value_t = 10)]
        k: usize,
        ...
        #[arg(long, default_value_t = 0)]
        qa_limit: usize,
        /// Base URL for HTTP systems (or MNEMOSYNE_VS_URL)
    ...
    /// (MNEMOSYNE_LLM_URL/MODEL/API); measures baseline vs augmented R@k on
...
fn fresh_store_id(level: SecurityLevel, id: &str) -> Result<(tempfile::TempDir, PalaceStore)> {
    ...
    let mut store = match std::env::var("MNEMOSYNE_EMBEDDER").as_deref() {
        ...
        // A model served over HTTP (`MNEMOSYNE_EMBED_URL`) — no feature gate,
...
fn run_xlingual(pairs_path: &str, limit: usize, level: SecurityLevel) -> Result<()> {
    ...
    let embedder = std::env::var("MNEMOSYNE_EMBEDDER").unwrap_or_else(|_| "hash".into());
    let (_tmp, mut store) = fresh_store(level)?;
...
const TAG_VOCAB: &[&str] = mnemosyne_core::KIND_VOCAB;
...
fn run_tagcost(
    ...
    let kind = match llm_api {
        "openai" => mnemosyne_llm::ApiKind::OpenAi,
        _ => mnemosyne_llm::ApiKind::Ollama,
    };
    let client = mnemosyne_llm::LlmClient::new(url, llm_model, kind)
...
fn run_wingscale(
    ...
    for floor_spec in floors.split(',').map(str::trim).filter(|s| !s.is_empty()) {
        let floor_label = if floor_spec.eq_ignore_ascii_case("off") {
            ...
            store.set_wing_pq_min(mnemosyne_store::WING_PQ_MIN_DEFAULT);
            format!("default({})", mnemosyne_store::WING_PQ_MIN_DEFAULT)
        } else {
...
/// the identical loop. Sealed level, embedder from `MNEMOSYNE_EMBEDDER`
...
fn locomo_eval(
    ...
    for (si, sample) in samples.iter().enumerate() {
        ...
        while let Some(dialogs) = conv.get(format!("session_{n}")).and_then(Value::as_array) {
            ...
            let mut ccur = 0usize;
            let copts = mnemosyne_core::ChunkOptions {
                chunk_size,
                ..Default::default()
            };
            ...
            let pieces: Vec<String> = if turn_units {
                turns
                    .iter()
                    .map(|(_, l)| mnemosyne_core::normalize_content(l))
                    .filter(|s| !s.trim().is_empty())
                    .collect()
            } else {
                mnemosyne_core::chunk_text(&body, copts)
            };
        ...
        if let Some(idx) = index.as_mut() {
            store.index_push(idx.as_mut(), mnemosyne_store::PlaintextPush::Allow)?;
        }
...
fn distill_eval(samples: &[Value], k: usize, qa_limit: usize) -> Result<()> {
    let llm = mnemosyne_llm::LlmClient::from_env()
        .map_err(|e| anyhow::anyhow!("distill gate needs a local LLM (MNEMOSYNE_LLM_URL): {e}"))?;
    ...
    for (si, sample) in samples.iter().enumerate() {
            ...
            match llm.extract_triples(&d.content) {
                Ok(triples) => {
                    for tri in triples {
                        ...
                        if mnemosyne_core::validate_name(&subj, "subject").is_err()
                            || mnemosyne_core::validate_name(&pred, "predicate").is_err()
                        {
                            continue;
                        }
        ...
        verified_total += store
            .kg_verify_receipts()?
            .iter()
            .filter(|r| matches!(r.verdict, mnemosyne_store::ReceiptVerdict::Verified))
            .count() as u32;
    ...
    match cli.command {
        ...
        Command::Vs {
            ...
            let sys: &mut dyn vs::MemorySystem = match system.as_str() {
                ...
                other => anyhow::bail!("unknown system {other:?} (mnemosyne|mem0|supermemory)"),
            };
